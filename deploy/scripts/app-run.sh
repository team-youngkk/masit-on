...
case "$component" in
  backend)
    ...
    YOUTUBE_BACKFILL_RUN_INTERVAL_SECONDS=$(optional_param /masiton/ai/youtube-backfill/run-interval-seconds); export YOUTUBE_BACKFILL_RUN_INTERVAL_SECONDS="${YOUTUBE_BACKFILL_RUN_INTERVAL_SECONDS:-0}"
    ...
    exec /usr/bin/docker run --name masiton-backend \
      ...
      -e YOUTUBE_BACKFILL_ENABLED -e YOUTUBE_BACKFILL_RUN_INTERVAL_SECONDS -e YOUTUBE_BACKFILL_QUOTA_WINDOW \
