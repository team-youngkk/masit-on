...
if [ -f "$REAL_IP_CONF" ]; then cp -p "$REAL_IP_CONF" "$ROLLBACK_DIR/real-ip.conf"; fi
...
rollback_basic_auth() {
  ...
  restore_or_remove real-ip.conf "$REAL_IP_CONF"
...
# ALB가 추가한 X-Forwarded-For만 VPC/ALB 경계에서 신뢰한다. 값이 없는 단일
# EC2에서는 loopback만 신뢰해 외부가 보낸 XFF로 client IP가 바뀌지 않게 한다.
{
