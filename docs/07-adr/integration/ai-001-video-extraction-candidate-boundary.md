...
세 소유자의 승인이 달리면 병합 직전 커밋에서 이 표시와 다른 문서의 같은 표시를 제거해 Accepted로 확정한다. 승인 없이 병합하지 않는다. 합의가 불발되면 해당 결정과 이를 근거로 추가한 `BR-AIEXTRACT-009`·`010`·`011`, API 계약의 `registrationUnits`·등록 단위 일괄 등록·`supplements`·`candidateTruncated`, 데이터 계약의 `ai_registration_unit`·`food_category_mapping`·`candidate_truncated`를 함께 되돌린다. 그 앞의 결정은 종전대로 Accepted다.
