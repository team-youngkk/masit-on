...
- MVP 범위에는 관리자 등록을 위한 메시지 큐나 비동기 워커가 없으므로([ADR-AUTO-001](../adr-backlog.md#adr-auto-001-자동-수집과-배치-처리)의 최근 기록 cleanup Scheduler 예외는 등록 경로에 적용하지 않음), 실패한 등록은 관리자가 나중에 수동으로 재시도하는 것이 유일한 복구 경로다.
