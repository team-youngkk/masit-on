...
related_documents:
  ...
  - ../../../infra/production/server-file-config-runbook.md
supersedes: []
superseded_by: null
---
...
### 1.1. 제3자 SSH 서버 파일 설정 예외 (2026-09-06 사용자 승인)

사용자가 이번 작업에서 승인한 예외는 제3자가 관리하는 SSH 배포 대상 앱 서버에만 적용한다. 기존 AWS 기본 결정과 ADR-SEC-002의 별도 Redis 서버 부트스트랩은 유지한다. 이 예외는 운영 전환·백업·복구 검증의 완료 기록이 아니다.

- 비밀이 아닌 endpoint·포트·사용자명·기능 설정은 `/etc/masiton/app.env`에 둔다. 비밀번호·API 키·서명 키·토큰은 넣지 않는다.
- 운영 비밀은 Git·이미지·CI 산출물 밖의 `/etc/masiton/secrets`에 속성별 파일로 보관하고, 필요한 컨테이너에만 읽기 전용으로 마운트한다. 이 경로의 호스트 평문 파일 보관에 한해 11절의 평문 저장 금지에 예외를 허용한다. 로그·응답·커밋·컨테이너 환경 변수로의 비밀 노출은 계속 금지한다.
- 호스트 관리자(root 또는 동등한 sudo/Docker 권한 보유자)는 비밀을 읽을 수 있다. 파일 권한과 읽기 전용 마운트는 관리자의 열람을 차단하거나 저장 암호화를 제공하지 않는다. 운영자는 이 신뢰 경계와 서버 침해 시 노출 위험을 수용한다.
- 해당 앱 런타임은 IAM Role·IMDS·SSM·KMS·S3 비밀 조회·ACM에 의존하지 않고 로컬 파일만 사용한다. AWS 자격 증명으로의 자동 fallback은 허용하지 않는다. 기존 IAM 정책이나 인스턴스 프로파일의 분리·삭제 여부는 별도 확인 대상이며 이 승인으로 완료를 주장하지 않는다.
- TLS 인증서와 개인 키는 로컬 Nginx에서 사용하며 발급·갱신은 운영자가 별도 관리한다. 앱 배포가 ACM export나 인증서 갱신을 수행하지 않는다.
- 서비스 중지·활성 산출물 교체 전에 설정·비밀 파일·권한·TLS 사전 점검을 통과해야 한다. 기존 이미지 digest 배포, health check와 실패·중단 시 rollback 계약을 유지한다.
- 서버 밖 암호화 백업과 수동 교체·복구 절차를 필수 운영 책임으로 둔다. 복호화 키는 서버 및 백업 저장소와 분리하고, 백업 성공과 복원 검증은 실제 증거가 있을 때만 기록한다. 구체 절차는 [파일 설정 운영 런북](../../../infra/production/server-file-config-runbook.md)을 따른다.

서버 소유자·접근 권한·배포 토폴로지가 바뀌거나 백업 복원이 실패하거나 비밀 유출이 의심되면 예외를 재검토한다. 아래 AWS 결정은 이 예외 밖의 기본 기준이다.
