...
이 ADR은 측정 **수단**을 확정하고, 실행 결과는 별도 결과 문서에 기록한다. 정상 부하는 Verified로 판정했고 최대 부하는 관찰 결과로 남아 있다.

| 공백 | 상태 | 계약 근거 |
|---|---|---|
| 정상 부하 50명·20 RPS 실측 | **Verified.** 2026-08-15 이슈 #207 격리 환경에서 공개 조회 세 endpoint의 p95·5xx·dropped 기준을 통과했다 | [RV-NFR-011](../../01-requirements/non-functional-requirements.md#rv-nfr-011-성능-테스트-환경), `NFR-PERFORMANCE-006` |
| 최대 부하 200명·80 RPS 시나리오와 실측 | **관찰 완료.** 같은 실행에서 80 RPS를 관찰했으며 정상 부하 정식 판정을 대체하지 않는다 | `RV-NFR-011`이 "정상 부하와 최대 부하를 **각각 실행한다**"로 확정 |

`RV-NFR-011`은 두 부하를 모두 실행하도록 이미 확정한 상태다. 최대 부하는 이번 실행에서 관찰했지만, 정상 부하와 동일한 정식 판정 근거로 사용하지 않는다.

최대 부하 정식 판정과 운영 인스턴스의 누적 데이터·동시 트래픽·캐시 워밍 상태를 반영한 별도 실측은 이 결과에 포함하지 않는다. 이번 정상 부하 결과는 [2차 확장 성능 검증 결과](../../08-planning/second-expansion-performance-verification.md)에 기록한다.
...
- `NFR-PERFORMANCE-006`의 정상 부하 판정은 2026-08-15 이슈 #207 격리 실행으로 완료됐다. `TST-E2-PERF-001`의 정상 부하 보류 항목이 해제됐다.
