...
컨테이너 기반 통합 테스트는 단위 테스트보다 실행 시간이 길고, 각 워크스트림 담당자가 자신의 Fixture와 테스트 데이터를 관리해야 하는 부담이 생긴다. 이 비용은 단위 테스트를 기본 개발 루프로 삼고, 컨테이너 통합 테스트를 실제 제약·관계 동작으로 좁혀 제한한다. 단계별 완료 전에 실제 정합성 문제를 탐지하고 향후 최종 배포의 수동 복구 위험을 줄이기 위해 필요한 비용으로 본다.
