...
- 핵심: AI 결과를 후보 Snapshot으로 격리하고, 자동 필수값·근거·중복·Kakao·YouTube·Visit 검증을 통과한 뒤 관리자 승인 없이 정식 등록·공개한다. Gemini Free Tier 전용 global endpoint의 `gemini-3-flash-preview`, P1/S1을 사용하며 유료 호출은 금지한다.
...
| Google Gemini API | Accepted | Free Tier 전용 global endpoint, `gemini-3-flash-preview`, P1/S1, 원문 비저장·유료 호출 금지 | 운영 계정 billing 미연결·모델별 무료 quota 확인 |
