...
| [ADR-OBS-001](quality/obs-001-logging-observability.md) | 애플리케이션 로그와 운영 관측 기준 | Superseded | High | SLF4J, Logback, Actuator, CloudWatch | ADR-OBS-002로 운영 수집·알림 경계를 갱신 | [문서](quality/obs-001-logging-observability.md) |
| [ADR-OBS-002](quality/obs-002-local-operations-without-cloudwatch.md) | CloudWatch 제거 후 로컬 운영 관측 경계 | Accepted | High | SLF4J, Logback, Actuator, Docker local logs | 전체 운영 | [문서](quality/obs-002-local-operations-without-cloudwatch.md) |
...
| [ADR-DEPLOY-005](platform/deploy-005-asg-blue-green-rollout.md) | ASG 기반 Blue-Green 운영 배포 | Superseded | Critical | ALB, ASG, CodeDeploy, Redis, GitHub Actions | Blue-Green 토폴로지는 유지하고 배포 알람 경계만 ADR-DEPLOY-007로 대체 | [문서](platform/deploy-005-asg-blue-green-rollout.md) |
...
| [ADR-DEPLOY-007](platform/deploy-007-codedeploy-health-alarm-boundary.md) | CodeDeploy 전환 중 ALB 호스트 상태 알람과 배포 게이트 분리 | Accepted | Critical | ALB, CodeDeploy, CloudWatch | `UnHealthyHostCount`는 관측 전용, 실제 장애 게이트는 target 5xx·latency·Redis alarm | [문서](platform/deploy-007-codedeploy-health-alarm-boundary.md) |
