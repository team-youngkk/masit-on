...
| 회원 JWT·Refresh Token | 구현 범위 확정 | Accepted ADR | [ADR-AUTH-002](security/auth-002-member-jwt-refresh-token.md), [ADR-AUTH-006](security/auth-006-cookie-origin-defense.md) | 관리자 audience·principal·쿠키와 분리, Refresh·Logout Origin 방어 |
...
| Nginx | 경로 경계 결정 완료 (2026-07-27) | Accepted ADR | [ADR-WEB-003](platform/web-003-routing-boundary.md), [ADR-RUNTIME-001](platform/runtime-001-docker.md) | `/api/**`는 Spring Boot, 나머지 외부 경로는 Next.js, `/internal/**`은 외부 차단. 애플리케이션 포트 바인딩은 [ADR-WEB-005](platform/web-005-application-port-binding.md)가 별도로 소유한다 |
...
| 관리자 인증 API | [ADR-AUTH-001](security/auth-001-spring-security-jwt.md), [ADR-AUTH-006](security/auth-006-cookie-origin-defense.md), [ADR-WEB-003](platform/web-003-routing-boundary.md), [ADR-DATA-005](data/data-005-redis-refresh-token.md) | 세부 matcher, JWT Bearer, Redis Refresh Token 보안 쿠키와 Origin 방어 |
