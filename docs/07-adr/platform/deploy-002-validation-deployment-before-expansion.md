...
### 3.1. 배포 고도화 착수 시점 결정 (2026-07-28)

배포·인프라 최종 책임자 이우람([ownership.md](../../03-team/ownership.md))이 3차 확장 이후 Blue-Green 무중단 배포로 전환하는 배포 고도화 단계를 수행하기로 결정했다. 기존 문서는 ALB·ASG·Blue-Green을 "트래픽·가용성 운영 근거가 생길 때 재검토할 확장 경로"로만 두었으므로, 이 결정은 조건부 경로를 착수 시점이 정해진 후속 단계로 바꾼다.

이 결정은 [이슈 #34](https://github.com/team-youngkk/masit-on/issues/34)에서 합의한 범위(M2 선행 배포와 확장 단계별 필요 인프라 반영)를 넘어선다. 확정하는 것은 **착수 시점과 목표 방식**뿐이며, 다음은 확정하지 않는다.

- ALB·ASG의 구체적 토폴로지와 전환 절차
- 고도화 단계의 인프라 비용과 예산 반영 범위
- 무중단 배포 자동화의 상세 범위

위 세 항목은 배포 고도화 착수 시점에 별도 ADR로 결정한다. 착수 전 재검토가 필요하면 이 ADR을 대체하는 새 ADR로 처리한다.
...
- 3차 확장까지는 단일 인스턴스와 수동 복구를 유지한다. ALB·Blue-Green 무중단 배포와 ASG 다중 인스턴스는 3차 확장 이후 배포 고도화 단계에서 전환한다(3.1절 결정). 토폴로지와 전환 절차, Nginx의 경로 라우팅 책임을 ALB가 대체할지는 그 시점의 별도 ADR에서 결정한다.
...
3차 확장 이후 배포 고도화 단계는 이 ADR의 재검토 조건이 아니라 3.1절에서 착수 시점을 결정한 후속 단계다. ALB·Blue-Green·ASG 다중 인스턴스의 구체적 토폴로지·전환 절차·비용은 그 시점에 별도 ADR로 결정한다. 착수 전에 단일 인스턴스 구성이 충분하다는 판단이 서면 이 ADR을 대체하는 새 ADR로 착수 시점 결정을 되돌린다.
