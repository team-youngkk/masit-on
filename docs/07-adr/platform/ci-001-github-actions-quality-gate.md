...
이 ADR의 품질 게이트 원칙은 유지하되, 현재 운영 배포 경로는 M2 당시의 ECR·OIDC·SSM/CodeDeploy 경로에서 다음과 같이 바뀌었다.
...
- `ssh-deploy`는 `main` ref에서 push 또는 `workflow_dispatch`로만 실행되고 `environment: production` 승인을 거친다. AWS OIDC, ECR login, GitHub Actions의 SSM Run Command는 현재 운영 배포 job에 없다. 제3자 SSH 앱 서버는 [ADR-SEC-001의 승인 예외](../security/sec-001-secrets-workload-identity.md)에 따라 `/etc/masiton/app.env`의 비밀이 아닌 설정과 `/etc/masiton/secrets`의 읽기 전용 비밀 파일을 사용한다. 앱 runtime의 IAM·SSM·ACM 및 AWS 비밀 조회 의존을 없애는 기준이며, 실제 IAM 권한 분리 완료를 뜻하지 않는다. 로컬 TLS 발급·갱신은 앱 배포와 별도로 관리한다.
...
- SSH preflight는 public IPv4 host, SSH 사용자, private key·known_hosts 및 known_hosts 일치를 검사하고, bundle 업로드 전에 원격 Docker·`sudo -n`·Python 3·`systemctl`·필수 명령과 x86_64 아키텍처를 확인한다. 파일 설정 경로에 AWS CLI를 요구하지 않으며, 설정·비밀·권한·TLS 검사는 서비스 중지와 활성 파일 교체보다 먼저 통과해야 한다. 실패 시 기존 서비스를 유지한다.
- 앱 설정·비밀 파일과 TLS 개인 키는 CI bundle에 포함하거나 배포 과정에서 덮어쓰지 않는다. 운영자가 읽을 수 있는 호스트 비밀의 서버 밖 암호화 백업·수동 교체·복원 증거는 [파일 설정 운영 런북](../../../infra/production/server-file-config-runbook.md)에 따라 별도로 관리한다. 승인된 기준과 실제 배포 검증 결과를 구분한다.
...
> 역사 구분: 아래 기존 절의 ECR push, GitHub Actions OIDC, SSM/CodeDeploy cleanup과 관련된 문장은 2026-07 M2의 설계·검증 사실을 보존한 기록이다. 현재 운영 경로로 해석하지 않으며, 위 현재 운영 기준이 우선한다.
