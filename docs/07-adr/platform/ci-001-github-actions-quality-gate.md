...
GitHub Actions를 CI 기준으로 사용한다. 모든 단계에서 빌드·자동 테스트를 필수로 하고, M2부터 AWS 접근이 필요한 단계는 OIDC를 사용한다.
...
고정 런타임(JDK 21.0.12, Node 24.18.0)·Wrapper(Gradle 8.14.3)·잠금 파일을 사용하고, 단위·통합·계약 테스트 중 하나라도 실패하면 해당 커밋을 단계 완료 후보에서 제외한다. M2부터 AWS 접근 단계는 GitHub Actions OIDC로 발급된 단기 자격 증명만 사용한다.
