...
루트 경로 공유는 화면과 API가 추가될 때마다 Nginx allowlist를 동시에 수정해야 하고 `Accept` 헤더 같은 간접 신호에 의존하면 오동작 가능성이 있다. 별도 서브도메인은 경계가 명확하지만 CORS, 쿠키, 인증과 인증서 운영 범위를 늘려 향후 단일 EC2 최종 배포에 불필요한 부담을 준다.
