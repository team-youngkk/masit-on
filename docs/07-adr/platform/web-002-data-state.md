...
또한 TanStack Query는 정확한 버전이 아직 확정되지 않았다(기술 정책 4장, [adr-traceability.md](../adr-traceability.md)). 버전 미확정 상태에서 설치를 금지하고 있으므로, 이 결정은 도구의 존재와 역할만 확정하고 실제 설치·구현은 버전이 확정된 뒤에야 시작할 수 있다는 일정상의 제약을 안고 있다.
