...
| WS-03 | 2 | 유튜버 선택 목록·관계 유효성·유튜버 조건·중복 제거 | Medium | 높음 |
...
**결정:** WS-03을 유지한다. `FR-CREATOR-001`, `FR-CREATOR-003`과 `BR-SEARCH-007`의 완료 책임은 WS-03이 갖고, WS-01은 `FR-RESTAURANT-005`에 따라 WS-03이 제공하는 선택·판정 결과를 이름·지역·카테고리 조건과 결합한다. 사용자에게는 하나의 탐색 흐름으로 제공하되 구현 책임만 분리한다.
...
|---|---|---|---|
| Critical 후속 설계 항목 | 관리자 제품 정책은 확정, 인증 기술과 일부 NFR 측정값은 미확정 | 인증 전달·보호 계약과 미확정 수치의 책임자·결정 시점 | WS-04, 보안 검증 |
...
- FR-CREATOR-001과 FR-CREATOR-003의 인수 조건과 적용 비기능 요구사항을 충족한다.
...
| FR-CREATOR-003 | 유튜버 필터 선택 목록 조회 | WS-03 | WS-01, WS-04 | WS-03이 공개 유튜버 선택 계약을 제공하고 WS-01이 필터 화면에서 사용한다. |
...
- 현재 상태: 결정 완료 — WS-01 ~ WS-04의 4개 유지
...
- 현재 상태: 결정 완료 — WS-03이 관계 판정·유튜버별 조회를 소유하고 WS-01이 최종 조건을 조합
- 관련 요구사항:
  - FR-CREATOR-001
  - FR-RESTAURANT-005
- 선택지:
...
- 현재 상태: 결정 완료 — 하나의 WS-04 안에서 기본 데이터와 방문 관계 등록의 두 단계로 관리
- 관련 요구사항:
  - FR-ADMIN-002 ~ FR-ADMIN-004
  - FR-VISIT-001
- 선택지:
