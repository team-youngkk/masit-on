...
이 문서는 맛잇온 1차 MVP의 기능 요구사항과 도메인 경계를, 한 명의 최종 책임자가 사용자 가치 또는 관리자 업무를 처음부터 끝까지 완성할 수 있는 세로 단위 Workstream으로 변환한다. 이후 `roles.md`, `ownership.md`, PRD, API 계약, 데이터 모델, 구현 계획과 Task 분배는 이 문서의 책임 경계와 요구사항 배정을 기준으로 구체화한다.

이 문서의 Workstream은 기술 계층이나 도메인과 일대일 대응하지 않는다. Workstream은 여러 도메인의 협업을 포함할 수 있지만 각 도메인의 데이터와 정책 소유권은 `domain-boundaries.md`를 따른다. 확정되지 않은 API 구조, 저장 방식과 운영 기준은 권장안 또는 검토 항목으로 구분하며 임의로 확정하지 않는다.
...
- 1차 MVP에서 제외된 수정·삭제, 유튜버 상세, 지도, 개인화, 추천, 자동 수집과 AI 추출 기능은 포함하지 않는다.
...
|---|---|---|---|---|
| WS-01 맛집 탐색 | 이름·지역·음식 카테고리·유튜버 조건으로 공개 맛집 탐색 | Restaurant | 목록, 검색, 필터 조합, 페이지, 정렬 | WS-03의 유효 방문 관계 판정, WS-04 등록 데이터 |
| WS-02 맛집 상세 및 콘텐츠 조회 | 한 맛집의 기본 정보와 방문 유튜버·관련 영상을 한 흐름에서 확인 | Restaurant, Visit | 기본 상세, 주소·카테고리, 유튜버·영상 조합, 빈 콘텐츠 처리 | Creator·Video 표시 정보, WS-04 등록 데이터 |
| WS-03 유튜버 기반 탐색 | 특정 유튜버가 실제 방문한 공개 맛집 탐색 | Visit | 유효 관계 기반 맛집 조회, 중복 제거, 공개 상태 적용 | Restaurant·Creator·Video 상태와 표시 정보, WS-04 등록 데이터 |
| WS-04 관리자 데이터 등록 | 조회에 필요한 맛집·유튜버·영상·방문 관계를 검증 순서대로 등록 | 관리자 유스케이스, Restaurant, Creator, Video, Visit | 관리자 접근, 기본 데이터 등록, 관계 등록, 중복·참조 검증 | 공통 인증 기반, 각 도메인의 등록 정책 |
...
|---|---:|---|---|---|
| WS-01 | 7 | 목록·검색·필터·페이지·정렬 | High | 높음 |
| WS-02 | 6 | 기본 상세·조합 조회·빈 콘텐츠·공개 상태 | High | 높음 |
| WS-03 | 1 | 관계 유효성·유튜버 조건·중복 제거 | Medium | 높음 |
| WS-04 | 5 | 인증·필수값·중복·참조·등록 순서·조회 반영 | High | 보통 |
| 합계 | 19 | 기능 요구사항 전체 | - | - |

복잡도는 기능 개수만이 아니라 도메인 협업, 공개 상태, 데이터 정합성과 통합 책임을 함께 반영한다. WS-03은 요구사항 수는 적지만 WS-01과 WS-02가 재사용할 Visit 판정 계약을 제공하므로 단순 작업으로 보지 않는다.

### 3.3 구성 대안 판단

#### 맛집 탐색과 유튜버 필터

| 기준 | WS-01에 전부 포함 | WS-03으로 관계 기반 탐색 분리 |
|---|---|---|
| 사용자 흐름 | 한 탐색 화면과 직접 대응한다. | 독립 유튜버 탐색 가치를 제공하면서 같은 화면에서 조건으로 재사용할 수 있다. |
| Visit 의존성 | WS-01이 Restaurant와 Visit 규칙을 함께 변경할 위험이 있다. | WS-03이 관계 유효성과 공개 상태 판정을 캡슐화한다. |
| 충돌 가능성 | 목록과 관계 판정 파일을 한 담당자가 폭넓게 수정한다. | WS-01은 최종 조건 조합, WS-03은 유튜버 조건 후보 산출로 경계가 선명하다. |
| API 계약 | 단일 탐색 계약은 자연스럽지만 내부 책임이 커진다. | 외부 계약은 단일 탐색으로 유지할 수 있고 내부 판정 계약만 분리할 수 있다. |
| 독립 테스트 | Visit 데이터 없이는 유튜버 조건 테스트가 어렵다. | WS-03의 관계 판정과 WS-01의 조건 결합을 각각 테스트할 수 있다. |
| 4인 병렬 개발 | 독립 작업 하나가 줄어든다. | 네 개 Workstream 병렬 배치와 잘 맞는다. |
| MVP 복잡도 | 초기 계약 수는 적지만 결합 복잡도가 WS-01에 집중된다. | 최소 판정 계약이 필요하지만 소유권과 장애 범위가 명확하다. |

**권장안:** WS-03을 유지한다. `FR-CREATOR-001`과 `BR-SEARCH-007`의 완료 책임은 WS-03이 갖고, WS-01은 `FR-RESTAURANT-005`에 따라 WS-03이 제공하는 유효 맛집 식별 결과를 이름·지역·카테고리 조건과 결합한다. 사용자에게는 하나의 탐색 흐름으로 제공하되 구현 책임만 분리한다.
...
| 도메인 협업 | WS-02가 조합 책임을 명시적으로 가진다. | Restaurant와 Visit 사이 통합 책임자가 추가로 필요하다. |
...
| 테스트·장애 격리 | 하위 계약을 독립 테스트하고 WS-02가 조합·부분 실패를 검증한다. | 하위 장애는 잘 격리되지만 빈 콘텐츠 정상 처리의 소유가 모호해진다. |

**권장안:** WS-02 하나로 유지한다. WS-02는 조합 유스케이스와 사용자 계약을 소유하지만 Restaurant, Visit, Creator와 Video의 내부 정책을 가져오지 않는다. 상세 기본 정보는 관계가 없어도 제공하고, 유효 관계·유튜버·영상이 없으면 빈 목록을 정상 결과로 제공한다.
...
**권장안:** WS-04 하나로 유지하되 내부를 `기본 데이터 등록`과 `방문 관계 등록`의 두 단계로 나눈다. WS-04는 인증·진입·순서·오류 조합을 책임지고, Restaurant·Creator·Video·Visit는 각자의 필수값·중복·관계 불변 조건을 책임진다. 역할 배정 시 WS-04의 높은 작업량을 공통 인증 기반 담당과 테스트 지원으로 보완한다.
...
|---|---|---|---|
| Critical 미확정 항목 | 관리자 인증 수준과 일부 NFR 측정값은 미확정 | 미확정 항목의 책임자·결정 시점, MVP 최소 인증 기준 | WS-04, 보안 검증 |
| 유튜버 관리 단위 | YouTube 채널을 고유 단위로 사용 | 채널 식별·표시 정보 계약 | WS-02, WS-03, WS-04 |
| 맛집·음식 카테고리 | 사전 정의된 대표 카테고리 정확히 1개 | 카테고리 값과 `기타` 기록 검증 계약 | WS-01, WS-02, WS-04 |
| 지역 관리 단위 | 서울특별시 전체 도로명주소와 자치구 1개 | 자치구 입력·표시·검증 계약 | WS-01, WS-02, WS-04 |
...
| 페이지네이션 | 크기 10·20·50, 기본 20, 범위 밖은 빈 목록 | 페이지 입력과 메타데이터 계약 | WS-01, WS-03 |
| 날짜·시간 | 방문 날짜는 MVP에서 관리하지 않음 | 공통 날짜·시간 표현과 시간대 원칙 | 등록·운영 공통 |
| 핵심 관계 | 방문 관계는 맛집·유튜버·영상 각 1개를 연결 | 존재·채널 일치·중복·유효성 판정 계약 | WS-02, WS-03, WS-04 |
| 관리자 권한 | 사전 발급 계정, 동일 등록 권한 | 인증 주체 전달과 접근 거부 계약 | WS-04 |
...
- 서울특별시 자치구 1개 필터
- 대표 음식 카테고리 1개 필터
- WS-03의 유효 관계 판정을 이용한 유튜버 1명 필터
- 검색과 필터의 AND 조합
...
- Creator, Video: WS-03을 통해 유튜버·영상 공개 상태 판정에 간접 참여

### 관련 요구사항

- FR-RESTAURANT-001 ~ FR-RESTAURANT-007

### 관련 비즈니스 규칙

- BR-RESTAURANT-001, BR-RESTAURANT-002, BR-RESTAURANT-008
- BR-SEARCH-001 ~ BR-SEARCH-009
- BR-PUBLICATION-001, BR-PUBLICATION-003 ~ BR-PUBLICATION-006

### 관련 비기능 요구사항

- NFR-PERFORMANCE-001, NFR-PERFORMANCE-002, NFR-PERFORMANCE-004
- NFR-SECURITY-002, NFR-SECURITY-003
- NFR-RELIABILITY-001, NFR-RELIABILITY-003
- NFR-COMPATIBILITY-002, NFR-COMPATIBILITY-003
- NFR-TEST-001, NFR-TEST-002

### 필요한 입력 데이터

- 공개 맛집 기본 정보, 자치구, 대표 음식 카테고리
- 공개·유효 관계에서 산출한 맛집별 YouTube 채널 표시 정보
- 선택한 유튜버에 대해 WS-03이 판정한 맛집 식별 결과
...
- WS-03의 관계 판정이 필요하지만 계약 Stub으로 유튜버 조건을 제외한 범위를 독립 개발할 수 있다.
- WS-04가 운영 데이터를 생성하지만 테스트 Fixture로 개발을 선행할 수 있다.
...
- WS-03의 유튜버별 유효 맛집 결과와 목록 채널명
- WS-04가 등록한 공개 맛집과 방문 관계의 조회 반영
- 공통 응답·오류·보안 계약
...
- WS-03 계약과 WS-04 등록 데이터가 실제 탐색 결과에 반영된다.
- 계약 문서와 실제 응답이 일치한다.
...
WS-02는 최종 상세 조합의 Workstream 소유자다. 실제 조합을 Restaurant 애플리케이션 책임 또는 전용 조회 책임 중 어디에 둘지는 후속 아키텍처 결정으로 남긴다.

### 관련 요구사항

- FR-RESTAURANT-008 ~ FR-RESTAURANT-011
- FR-CREATOR-002
- FR-VIDEO-001

### 관련 비즈니스 규칙

- BR-RESTAURANT-002, BR-RESTAURANT-004, BR-RESTAURANT-005, BR-RESTAURANT-008
- BR-CREATOR-004, BR-CREATOR-007
- BR-VIDEO-001, BR-VIDEO-004, BR-VIDEO-007 ~ BR-VIDEO-009
- BR-VISIT-004, BR-VISIT-005
- BR-PUBLICATION-001, BR-PUBLICATION-003 ~ BR-PUBLICATION-007
...
- NFR-INTEGRITY-004
- NFR-RELIABILITY-001, NFR-RELIABILITY-003
- NFR-EXTERNAL-001, NFR-EXTERNAL-002
- NFR-COMPATIBILITY-002, NFR-COMPATIBILITY-003
- NFR-TEST-001, NFR-TEST-002
...
- WS-03과 Visit의 관계 유효성 판정 규칙을 공유하지만, WS-03의 완성 API 구현에는 의존하지 않는다.
- WS-04 등록 데이터가 필요하지만 Fixture와 Fake 제공자로 독립 개발할 수 있다.
...
4. WS-01이 이 결과를 다른 탐색 조건과 결합하여 최종 목록과 페이지를 제공한다.
...
- 근거 영상이 없는 추정 방문
- 독립 유튜버 목록 조회: MVP 포함 여부가 미결정이므로 현재 구현 범위에 넣지 않는다.
...
- BR-CREATOR-001, BR-CREATOR-004, BR-CREATOR-005, BR-CREATOR-007
- BR-VIDEO-005, BR-VIDEO-009
- BR-VISIT-001 ~ BR-VISIT-007
- BR-SEARCH-003 ~ BR-SEARCH-007
- BR-PUBLICATION-001, BR-PUBLICATION-003 ~ BR-PUBLICATION-006
...
- NFR-RELIABILITY-001, NFR-RELIABILITY-003
- NFR-TEST-001, NFR-TEST-002
- NFR-MAINTAINABILITY-001, NFR-MAINTAINABILITY-002
...
- WS-01이 다른 탐색 조건과 결합할 수 있는 맛집 식별 결과

### 선행 조건

- YouTube 채널 단위 유튜버 식별 계약
- 네 대상의 공개 상태와 Visit 유효성 계약
- WS-01과 공유할 유튜버 조건 판정 계약

### 다른 Workstream 의존성

- WS-01과 목록 표현·페이지 계약을 공유하되 최종 조건 결합은 WS-01이 책임진다.
- WS-02와 관계 유효성 규칙을 공유하되 서로의 구현을 호출하지 않는다.
- WS-04가 관계 데이터를 생성하지만 Fixture로 독립 개발할 수 있다.
...
- WS-01의 최종 검색·필터 AND 조합
- WS-02의 맛집별 연결 콘텐츠 조회와 동일한 관계 유효성 규칙
- WS-04의 방문 관계 등록 결과
...
- WS-01의 이름·지역·카테고리 조건과 결합

### 완료 조건

- FR-CREATOR-001의 인수 조건과 적용 비기능 요구사항을 충족한다.
- 관계 유효성 판정이 WS-02와 동일한 정책을 사용한다.
- WS-01이 실제 계약으로 결과를 결합하고 중복 없는 목록을 제공한다.
- 비공개·무효 관계가 사용자 결과에 노출되지 않는다.
- 자동화 테스트와 계약 문서가 완료된다.

### 주요 리스크

- 유튜버 선택지를 얻는 목록 기능이 별도 요구사항으로 정의되지 않았다.
- WS-01과 Visit 내부를 공유하면 순환 구현 의존이 생길 수 있으므로 판정 계약 경계를 지켜야 한다.
...
6. 등록 결과가 WS-01, WS-02와 WS-03의 사용자 조회에 반영된다.
...
- FR-ADMIN-001 ~ FR-ADMIN-004
- FR-VISIT-001

### 관련 비즈니스 규칙

- BR-RESTAURANT-003 ~ BR-RESTAURANT-008
- BR-CREATOR-001 ~ BR-CREATOR-003, BR-CREATOR-005
- BR-VIDEO-001 ~ BR-VIDEO-006
- BR-VISIT-001 ~ BR-VISIT-007
- BR-ADMIN-001 ~ BR-ADMIN-008
- BR-PUBLICATION-001, BR-PUBLICATION-002, BR-PUBLICATION-008

### 관련 비기능 요구사항

- NFR-PERFORMANCE-003
- NFR-SECURITY-001 ~ NFR-SECURITY-003
- NFR-INTEGRITY-001 ~ NFR-INTEGRITY-003
- NFR-EXTERNAL-003
- NFR-OBSERVABILITY-001 ~ NFR-OBSERVABILITY-003
- NFR-TEST-001 ~ NFR-TEST-003
- NFR-PRIVACY-001, NFR-PRIVACY-002
...
- WS-01, WS-02와 WS-03에서 조회 가능한 공개 데이터
...
- 조회 반영 인수 조건은 WS-01, WS-02와 WS-03의 계약 또는 검증용 읽기 경로가 필요하다.
...
- 내용:
  ...
  - WS-01은 상세 진입에 필요한 식별자를 제공하고 WS-02는 그 식별자로 상세를 완성한다.
  - FR-CREATOR-002의 주 책임인 WS-02는 맛집별 방문 채널 표시 계약을 제공하고 WS-01은 목록 표현과 페이지 조합을 책임진다.
...
- 내용:
  - WS-03은 유튜버 조건에 맞는 공개·유효 맛집 식별 결과를 제공한다.
  - WS-01은 그 결과를 이름·지역·카테고리 조건과 결합하고 최종 목록·페이지를 소유한다.
...
- 내용:
  ...
  - WS-02는 맛집에서 콘텐츠로, WS-03은 유튜버에서 맛집으로 조회 방향만 다르다.
...
- 구분: 데이터 및 운영 의존성
- 내용: WS-04가 등록한 공개 맛집과 관계 데이터가 목록·검색·필터에 반영된다.
...
구현 변경 방향은 `WS-04의 조율 → 각 소유 도메인`과 `조회 Workstream → 도메인이 공개한 조회 계약`으로 유지한다. 조회 결과가 등록 기능을 호출하거나, Restaurant와 Visit가 서로의 내부 모델을 변경하는 순환은 허용하지 않는다.
...
|---|---|---|
| 개발자 A | WS-01 | 공통 페이지네이션 계약 |
| 개발자 B | WS-02 | 조회 조합 계약 |
| 개발자 C | WS-03 | Visit 조회·유효성 규칙 |
| 개발자 D | WS-04 | 공통 관리자 권한 기반 |

이 표는 최종 Role 배정안이 아니라 네 Workstream이 4인 병렬 개발에 적합한지 검증하기 위한 예시다. 실제 담당자와 소유 파일은 `roles.md`와 `ownership.md`에서 확정한다.

각 개발자는 주 Workstream의 요구사항 구체화, 계약 상세화, 데이터 접근, 비즈니스 로직, 오류 처리, 자동화 테스트, 문서 갱신과 통합 지원을 끝까지 책임진다. WS-04의 작업량이 크므로 개발자 C는 관계 유효성 계약, 개발자 B는 등록 후 상세 반영 검증처럼 계약 소유 범위 안에서 통합을 지원할 수 있다.
...
| 관리자 인증·권한 기반 | 등록 기능 접근 통제 | WS-04 등록 연결 전 | 최소 정책은 예 | 필요 | WS-04, 공개 조회 보안 | 인증 성공·실패·우회 테스트 통과 |
| 마이그레이션 기반 | 한 저장소의 변경 순서와 재현성 보장 | 데이터 구현 전 | 규칙은 예 | 필요 | 전체, 특히 WS-04 | 빈 환경과 기존 환경에서 동일 적용 가능 |
...
|---|---|---|---|---|
| FR-RESTAURANT-001 | 맛집 목록 조회 | WS-01 | WS-03, WS-04 | 탐색 시작점이며 채널명과 등록 데이터가 필요하다. |
| FR-RESTAURANT-002 | 맛집 이름 검색 | WS-01 | WS-04 | Restaurant 탐색 규칙이며 등록된 이름을 사용한다. |
| FR-RESTAURANT-003 | 지역별 필터 | WS-01 | WS-04 | Restaurant가 소유하는 자치구 조건이다. |
| FR-RESTAURANT-004 | 음식 카테고리별 필터 | WS-01 | WS-04 | Restaurant가 소유하는 대표 카테고리 조건이다. |
| FR-CREATOR-001 | 유튜버 기준 방문 맛집 조회 | WS-03 | WS-01, WS-04 | Visit의 유효 관계 판정이 핵심이고 WS-01이 결과를 조합한다. |
| FR-RESTAURANT-005 | 검색 및 필터 조건 조합 | WS-01 | WS-03 | 최종 AND 조합과 맛집 목록 완료 책임은 WS-01에 있다. |
| FR-RESTAURANT-006 | 페이지 단위 조회 | WS-01 | 없음 | 최종 탐색 결과의 페이지 정책이다. |
| FR-RESTAURANT-007 | 기본 정렬 적용 | WS-01 | 없음 | 최종 맛집 목록의 안정 정렬 책임이다. |
| FR-RESTAURANT-008 | 맛집 기본 정보 조회 | WS-02 | WS-04 | 상세 가치의 기본 정보이며 등록 데이터가 필요하다. |
| FR-RESTAURANT-009 | 지역 정보 확인 | WS-02 | WS-04 | 상세 화면의 전체 도로명주소 제공 책임이다. |
| FR-RESTAURANT-010 | 음식 카테고리 확인 | WS-02 | WS-04 | 상세 화면의 대표 카테고리 제공 책임이다. |
| FR-RESTAURANT-011 | 영상 연결이 없는 맛집 상세 조회 | WS-02 | WS-04 | 기본 상세와 빈 콘텐츠의 조합 책임이 WS-02에 있다. |
| FR-CREATOR-002 | 방문 유튜버 정보 확인 | WS-02 | WS-01, WS-03, WS-04 | 맛집 상세·목록 맥락의 관계 기반 유튜버 표시 기능이며 WS-02가 공통 표시 계약을 완결한다. |
| FR-VIDEO-001 | 관련 영상 정보 확인 | WS-02 | WS-03, WS-04 | 상세 화면에서 관계 기반 영상을 조합하는 기능이다. |
| FR-ADMIN-001 | 관리자 등록 기능 접근 | WS-04 | 공통 인증 작업 | 관리자 등록 흐름 전체의 선행 접근 책임이다. |
| FR-ADMIN-002 | 맛집 정보 등록 | WS-04 | WS-01, WS-02 | 관리자 데이터 생성 흐름이며 두 조회에 반영된다. |
| FR-ADMIN-003 | 유튜버 정보 등록 | WS-04 | WS-02, WS-03 | 관계 생성의 선행 데이터이며 유튜버 조회에 반영된다. |
| FR-ADMIN-004 | 영상 정보 등록 | WS-04 | WS-02, WS-03 | 관계 근거의 선행 데이터이며 상세·필터 유효성에 쓰인다. |
| FR-VISIT-001 | 맛집·유튜버·영상 방문 관계 등록 | WS-04 | WS-01, WS-02, WS-03 | 관리자 등록 흐름의 마지막 단계이며 모든 관계 기반 조회에 반영된다. |

기능 요구사항 19개가 모두 한 번씩 주 Workstream에 배정되었으며 중복 주 배정이나 미배정 요구사항은 없다.
...
| 3. Workstream별 독립 개발 | 단계 2 | WS-01 ~ WS-04 모두 | 각 WS가 Fixture·Stub/Fake로 자동화 테스트 통과 |
| 4. 기본 데이터 등록 통합 | WS-04 기본 등록, 공통 인증·마이그레이션 | WS-01 기본 탐색, WS-02 기본 상세 계속 | 맛집·유튜버·영상 등록 및 기본 조회 반영 |
| 5. 조회 Workstream 통합 | Restaurant 표시 계약 | WS-01 목록과 WS-02 상세 병렬 통합 | 목록에서 상세 진입, 기본 정보 일치 |
| 6. 방문 관계 기반 조회 통합 | WS-03 관계 판정, WS-04 관계 등록 | WS-01 유튜버 조건과 WS-02 콘텐츠 조합 병렬 | 등록 관계가 세 조회에 동일 정책으로 반영 |
| 7. 공통 보안·오류 검증 | 통합 API | 모든 WS의 실패·경계 테스트 | 인증, 비공개, 오류·부분 실패 정책 통과 |
| 8. 전체 인수 테스트 | 단계 4 ~ 7 | 성능·호환성·관찰성 검증 병행 | 기능 요구사항 19개와 MVP 시나리오 통과 |
| 9. 배포 검증 | CI 품질 게이트 | 운영 문서 최종화 | 배포 전후 검사, 헬스체크와 복구 절차 확인 |

첫 통합 대상으로 `WS-04 기본 맛집 등록 → WS-01 유튜버 조건 없는 기본 목록`을 권장한다. 가장 적은 관계 의존으로 등록 데이터의 조회 반영, 공개 상태, 페이지와 기본 오류 계약을 조기에 검증할 수 있기 때문이다. 이후 WS-02 기본 상세를 붙이고 마지막에 Visit 관계 기반 세 흐름을 동시에 연결한다.

## 14. MVP 완료 기준

- 기능 요구사항 19개가 하나의 주 Workstream에 배정되어 있고 각 인수 조건을 충족한다.
...
- 관리자 등록 데이터가 WS-01, WS-02와 WS-03에 반영된다.
...
- `roles.md`와 `ownership.md`에서 각 Workstream 최종 책임자와 계약·공유 파일 소유자를 지정할 수 있다.

## 15. 검토 필요 항목

### RV-WS-001 실제 Workstream 개수

- 현재 상태: 본 문서 권장안 4개, Role 배정 전 팀 확인 필요
- 관련 요구사항: 기능 요구사항 19개 전체
- 선택지:
  1. 권장안대로 WS-01 ~ WS-04 유지
  2. WS-04의 기본 데이터 등록과 방문 관계 등록을 분리
- 영향: 책임 완결성, WS-04 작업량, 팀원 배치와 통합 일정
- 결정 시점: Role 배정 전

### RV-WS-002 유튜버별 필터의 Workstream 소유권

- 현재 상태: WS-03 주 소유, WS-01 최종 조건 조합을 권장하며 팀 확인 필요
- 관련 요구사항:
  - FR-CREATOR-001
  - FR-RESTAURANT-005
- 선택지:
  1. WS-01이 관계 판정까지 소유
  2. WS-03이 관계 판정·유튜버별 조회를 소유하고 WS-01이 결과를 조합
- 영향: Visit 의존성, API 계약, 독립 테스트와 통합 일정
- 결정 시점: Role 배정 및 API 명세 전

### RV-WS-003 맛집 상세 조합 조회의 책임 위치

- 현재 상태: WS-02 Workstream 소유는 권장, 실제 애플리케이션 책임 위치는 팀 결정 필요
- 관련 요구사항:
  - FR-RESTAURANT-008 ~ FR-RESTAURANT-011
  - FR-CREATOR-002
  - FR-VIDEO-001
...
- 현재 상태: 팀 결정 필요, 결정 전에는 WS-03 범위에서 제외
- 관련 요구사항:
  - FR-CREATOR-001
- 선택지:
  1. 기존 필터 계약의 보조 데이터로만 정의
  2. 별도 기능 요구사항으로 추가한 뒤 WS-03에 배정
...
- 관련 요구사항:
  - FR-ADMIN-002 ~ FR-ADMIN-004
  - FR-VISIT-001
- 결정 내용: 등록만 포함하며 수정·삭제·승인 상태 관리는 구현하지 않는다.
- 영향: WS-04 범위와 정정 운영 절차
- 근거: BR-ADMIN-005, RV-BR-009, RV-DOMAIN-008

### RV-WS-006 기본 데이터 등록과 방문 관계 등록의 분리 여부

- 현재 상태: 하나의 WS-04 안에서 두 단계로 분리 권장, 팀 확인 필요
- 관련 요구사항:
  - FR-ADMIN-002 ~ FR-ADMIN-004
  - FR-VISIT-001
- 선택지:
  1. 하나의 WS-04에서 단계와 도메인 계약만 분리
  2. 두 Workstream으로 분리
...
- 현재 상태: 최소 인증 정책과 구현 담당 모두 팀 결정 필요
- 관련 요구사항:
  - FR-ADMIN-001
  - NFR-SECURITY-001
- 선택지:
  1. WS-04 책임자가 공통 작업 구현도 담당
  2. 다른 개발자가 공통 구현을 보조하고 WS-04가 인수·적용 책임
- 영향: WS-04 작업량, 보안 검증과 공통 코드 소유권
- 결정 시점: 구현 시작 전

### RV-WS-008 Workstream 작업량 차이

- 현재 상태: WS-04가 가장 넓고 WS-03은 요구사항 수보다 통합 지원 비중이 큼
- 관련 요구사항: 기능 요구사항 전체
- 조정 원칙:
  - Workstream을 인위적으로 쪼개기보다 WS-03 담당자가 Visit 계약과 관계 기반 통합을 지원한다.
  - WS-01·WS-02 담당자는 자신이 소비하는 등록 결과의 인수 테스트를 소유한다.
- 영향: 보조 책임, 일정과 리뷰 부하
- 결정 시점: Role 배정 시
...
- 필요 지점:
  - WS-01의 WS-03 유튜버 조건 결과
  - WS-02의 Visit·Creator·Video 표시 정보
  - WS-03의 Restaurant 표시 정보
  - WS-01 ~ WS-03의 WS-04 등록 데이터
  - WS-04의 관리자 인증 제공자
...
- 현재 상태: WS-04 기본 맛집 등록 → WS-01 기본 목록 권장
- 관련 요구사항:
  - FR-ADMIN-002
  - FR-RESTAURANT-001
- 대안: WS-01 목록 → WS-02 기본 상세를 먼저 연결
...
- 관련 요구사항:
  - FR-RESTAURANT-001
  - FR-CREATOR-002
- 선택지: 전체 표시, 제한 후 축약 표시 또는 별도 상세 유도
...
- 관련 요구사항:
  ...
  - FR-VIDEO-001
- 선택지:
  1. 내부 제공자 실패 시 상세 전체 오류
  2. 정책상 허용된 경우 기본 상세와 명시적 부분 결과 제공
