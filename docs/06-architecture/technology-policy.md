...
- 1차 MVP는 단일 EC2 인스턴스(Nginx 리버스 프록시 + Spring Boot 애플리케이션)로 배포하며 다중 리전·다중 인스턴스 고가용성 구성을 필수로 하지 않는다.
