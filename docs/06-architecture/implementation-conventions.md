...
이 문서는 맛잇온 MVP에 참여하는 네 명의 개발자와 AI 에이전트가 공통으로 적용할 구현 규칙을 한곳에서 확인하기 위한 기준이다. 상세 설계나 계약을 이 문서에 중복 정의하지 않고 권위 있는 원문을 연결한다.
...
- 모든 PR은 작성자를 제외한 최소 한 명의 승인을 받아야 한다.
...
- AI가 작성한 코드도 동일한 PR, 테스트와 최소 한 명 승인 규칙을 적용한다.
