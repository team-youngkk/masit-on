...
이 차단은 Nginx 경로 규칙 하나에 의존하지 않는다. 운영 애플리케이션은 `server.address`를 loopback으로 고정해 인스턴스 밖에서는 애플리케이션 포트에 TCP 연결 자체가 성립하지 않게 한다. 보안 그룹이나 방화벽 규칙이 애플리케이션 포트를 열어도 Nginx를 건너뛴 직결과 `/internal/**` 노출이 생기지 않는다. 경계 구성과 배포 후 확인 명령은 [애플리케이션 포트 바인딩 경계](../08-planning/issue-200-application-port-binding.md)가 소유한다.
