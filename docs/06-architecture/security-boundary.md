...
회원 인증은 관리자와 별도 JWT audience(`masit-on-member-api`), `MEMBER` authority, `MemberPrincipal(memberId, sessionId)`를 사용한다. 공개 회원 인증 메서드를 제외한 `/api/auth/**`와 `/api/me/**`에는 회원 decoder만, `/api/admin/**`에는 관리자 decoder만 적용해 교차 audience를 인증 단계에서 거부한다.
