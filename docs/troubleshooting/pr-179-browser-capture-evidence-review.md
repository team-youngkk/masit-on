...
- 재발 방지: 검증 기록 8절 재현 절차에 캡처 생성 명령과 360·390px의 500px 최소 창 너비 우회를 추가했다. 다음 검증 Task가 같은 제약에 다시 걸리지 않는다.
- 다음 확인: 여정 상태와 관리자 화면 캡처, 지원 브라우저 실빌드의 여정 확인은 검증 기록 6절에 미검증으로 남아 있다. `E3-T13` 활성화 판정 전에 담당자가 채운다. 추적은 이슈 [#165](https://github.com/team-youngkk/masit-on/issues/165)에서 이어간다.
