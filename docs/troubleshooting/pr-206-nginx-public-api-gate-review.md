...
후속 리뷰에서는 설치 스크립트가 `nginx-smoke.sh`를 필수 파일로 요구하지만 실제 SSM 번들 생산자가 앱 배포 파일만 전송하는 문제도 확인됐다. 이 상태에서는 저장소의 Nginx 설치·smoke 변경이 운영 인스턴스에 전달되거나 실행되지 않는다.
...
Nginx smoke를 설치 스크립트에 연결하면서 소비자(`nginx-install.sh`)의 입력 계약만 갱신하고, 그 입력을 만드는 생산자(`.github/workflows/ci.yml`의 SSM 번들 목록)까지 추적하지 않았다. 그 결과 로컬 정적 테스트는 설치 스크립트 내부의 실행 순서만 확인했고, 실제 배포 경로의 파일 전달과 설치 호출 누락은 검출하지 못했다. 파일을 기존 방식대로 개별 base64 명령으로 추가한 첫 수정은 실제 SSM 명령 JSON을 약 92KB로 늘려 64KB 제한을 넘었으므로, 번들 전달 방식 자체도 함께 바꿔야 했다.
...
| `nginx-install.sh` 필수 파일 목록과 CI `files` 배열 대조 | Nginx 설치 산출물 8개가 전부 SSM 번들에서 누락되고 설치 호출도 없음 | 번들 생산자와 실행 순서를 함께 수정하고 정적 계약으로 고정 |
| `nginx-install.sh`의 사전 검증 순서 확인 | `127.0.0.1:8080/internal/verification/session`이 401이어야 컷오버 가능 | `app-deploy.sh` 성공 뒤 `nginx-install.sh`를 실행하도록 순서 고정 |
| Nginx 산출물을 개별 base64 명령으로 추가해 실제 CI Python 실행 | SSM 명령 JSON이 약 92KB로 증가 | 단일 `tar.gz` 압축 전송으로 바꾸고 직렬화 크기 상한을 CI에서 강제 |

## 6. 최종 해결

- 변경 내용: `nginx-smoke.sh`의 공통 라우팅 실패 조건에 `3[0-9][0-9]`를 추가해 공개 GET·POST·Webhook 모두 3xx에서 실패하게 했다. `AppRunScriptContractTest`가 해당 정규식과 공통 함수 사용을 강제한다. ADR-DEPLOY-003은 원결정을 보존한 채 `Superseded`로 전환하고, 신규 ADR-DEPLOY-004에 비관리자 `permitAll` API만 경로·Method 단위로 gate에서 제외하는 후속 결정과 트레이드오프·검증 항목을 정의했다. ADR 인덱스·추적표·기술 정책과 현재 OPS-VALIDATION 계약 참조를 ADR-DEPLOY-004로 전환했다. CI의 SSM 번들에는 Nginx 설치에 필요한 산출물 8개를 포함한 단일 `tar.gz`를 추가하고, 새 백엔드 배포가 성공한 뒤 `nginx-install.sh`가 smoke를 포함한 컷오버를 수행하도록 연결했다. 직렬화된 명령 JSON이 64KB를 넘으면 SSM 호출 전에 CI가 실패한다.
...
- 재발 방지: 공개 API smoke의 3xx 거부와 CI 번들의 Nginx 필수 산출물·실행 순서를 정적 계약 테스트에 추가했다. 배포 스크립트의 필수 입력이 바뀌면 소비자뿐 아니라 SSM 번들 같은 생산자도 함께 대조한다. 공개 API 목록 변경 시 Spring Security·Nginx·API 계약과 현재 Accepted ADR을 대조한다. Accepted ADR의 결론 변경이 필요하면 본문을 직접 고치기 전에 `docs/07-adr/README.md` 9절과 대체 메타데이터·인덱스·추적표를 먼저 확인한다.
