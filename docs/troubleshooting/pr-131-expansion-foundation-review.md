...
| [E2-T01 구현 게이트를 먼저 통과](https://github.com/team-youngkk/masit-on/pull/131#discussion_r3709848370) (P1, 김인안) | 선행 운영 검증과 owner 승인 뒤 권위 문서를 통과 상태로 동기화하거나 그 전까지 구현 PR 병합을 중단 | Git·운영 게이트 | 결정 필요 | 최신 재리뷰에서 Ready로 돌아간 PR을 다시 Draft로 전환해 병합을 차단하고, 코드나 계약 상태를 임의로 변경하지 않은 채 스레드를 미해결로 유지한다. | `implementation_gate: Blocked`, 운영 `/me`의 Basic Auth `401`, 4개 선행 PRD/API의 `status: draft`와 PR Draft 상태를 확인 |
...
- 실제 결과: 구현 코드와 V3가 존재하지만 `implementation_gate`는 `Blocked`이고, 선행 PRD/API 네 문서는 `draft`다. 운영 `/me`도 검증 세션 흐름 대신 Nginx Basic Auth `401`을 반환했으며, 한 차례 Draft 전환 뒤 PR 메타데이터가 다시 Ready로 바뀌어 있었다.
...
3.2는 이슈 #105가 닫힌 사실을 E2-T01 통과로 해석했지만, 실제 완료 PR #126은 자동 검증 결과만 보강하고 운영 검증과 owner 승인이 남아 `Blocked`를 유지한다고 명시한다. 이슈 상태와 권위 문서의 게이트 판정을 별개 증거로 확인하지 않은 것이 원인이다. Draft 전환 후 Ready로 돌아간 주체와 이유는 현재 증거만으로 확정할 수 없어 `추정하지 않음`으로 남기고, 최신 상태를 다시 Draft로 교정했다.
...
| `curl.exe -sS -D - -o NUL https://masiton.click/me` | `401 Unauthorized`, `WWW-Authenticate: Basic realm="masiton verification"` | E2-T01 운영 전환이 아직 완료되지 않았다는 재현 증거로 기록 |
| 최신 재리뷰 뒤 PR 메타데이터 재확인 | `isDraft=false`로 다시 변경돼 있었음 | 원인 주체는 추정하지 않고 `gh pr ready 131 --undo`로 Draft 상태 복원 |
...
- 변경 내용: 최신 재리뷰에서 PR이 다시 Ready임을 확인해 Draft 상태를 재적용하고 운영 Basic Auth `401` 증거를 기록했다.
...
| 운영 `/me` 응답 헤더 확인 | 확인, `401 Unauthorized` | 검증 세션 전환 전 Basic Auth가 아직 운영 요청을 차단함 |
| `gh pr view 131 --json isDraft` | 통과, `true` | Blocked 게이트와 PR Draft 상태가 일치함 |
...
| 운영 `/me` Basic Auth 응답 | `401` 1건 재현 | 인증정보 없는 `GET /me` 응답 헤더 | 확인 예정 | 검증 세션 전환 전이므로 개선 여부 판단 불가 | OPS-VALIDATION 담당, 게이트 재검증 시점 |
