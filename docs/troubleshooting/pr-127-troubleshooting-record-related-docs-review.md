...
- 실제 결과: "범위"는 "PR 본문 인라인 리뷰 스레드 N건"이라고 적었지만, 세 스레드의 `path`는 각각 `pr-125-develop-to-main-sync-policy-review.md`, `docs/troubleshooting/README.md`, `pr-127-troubleshooting-record-related-docs-review.md`였다. 즉 PR 설명(본문)이 아니라 변경된 파일의 diff 라인에 달린 리뷰였다.
...
- 선택 이유: 네 건 모두 판단 내용(정정 방향) 자체는 이미 옳았고, 근거·서술·검증의 정밀도만 실제 상태와 맞추면 되는 최소 수정이다.
...
| GraphQL로 스레드 3건의 `path` 필드 재조회 | 통과 | 세 스레드 모두 PR 본문이 아니라 변경 파일 diff 라인에 달렸음을 확인 |
