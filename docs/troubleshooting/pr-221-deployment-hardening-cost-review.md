...
| 작성자 | `w00lam` |
| 처리 일자 | 2026-08-17 |
| 범위 | `deployment-hardening-impact-review.md`의 미해결 인라인 리뷰 4건 |
...
- 실제 결과: Redis 기동 필수인 Parameter Store 접근이 외부 경로와 비용 표에서 빠져 E1·E2와 S2 판정이 낮게 계산됐다. E2 이미지 경로에는 S3 레이어 다운로드 경로가 빠졌고, S1 + E3의 최고 환율 환산값은 178,600원으로 재현되지 않았다. 성능 게이트 문장은 정상 `50/20` 재실행을 요구하는 것처럼 읽혔다.
...
비용 문서를 작성할 때 현재 앱 EC2의 퍼블릭 경로 전제를 전용 Redis의 사설 서브넷 경로에 그대로 축소 적용했다. 그 과정에서 사람이 관리 접속하는 경로만 세고, `redis-render-conf.sh`가 기동 때마다 호출하는 Parameter Store를 별도 런타임 의존성으로 분리하지 않았다. 또한 ECR 레이어 획득에 필요한 S3 게이트웨이 경로를 E1과 달리 E2 표에 기록하지 않았고, 환율 환산 한 칸과 원문 게이트 범위 서술을 후속 변경 때 함께 대조하지 않았다.
...
- 변경 내용: Parameter Store 접근 열과 `ssm` 엔드포인트 비용을 추가하고, E2의 S3 게이트웨이 엔드포인트, 모든 영향 절의 비용·판정, 1,559원 환산값, 성능 게이트의 실행·증거 재사용 범위를 갱신했다.
