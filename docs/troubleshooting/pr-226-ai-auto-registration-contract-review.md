...
|---|---|---|---|---|---|
| [MAX_CANDIDATES 자기모순 (P1)](https://github.com/team-youngkk/masit-on/pull/226#discussion_r3800261061) | 같은 절에서 상한 인상이 결정·미결로 동시 서술됨 | 기타 | 수정 필요 | 결정 요약의 단정 문장을 삭제하고 "이 결정은 수신 상한을 바꾸지 않는다"로 교체. 도입부에도 제외를 명시 | 9절 결정 요약·남은 결정·PR 본문 3곳 대조 |
| [MAX_CANDIDATES 결정 상태 정리 (P2)](https://github.com/team-youngkk/masit-on/pull/226#discussion_r3800264157) | 미결 유지 시 상한 초과 예외·대체 경로를 PRD/BR/API에 명시 | 기타 | 수정 필요 | 위 수정에 더해 `BR-AIEXTRACT-001`과 PRD 9절에 상한 초과 시 `SCHEMA` 기각·복구 경로를 추가 | 손실 분석 4.5절의 기존 상한 서술과 대조 |
...
- **추적 대상** — `합의 대기` 표시 제거는 김인안·박진영·restaurant 도메인 소유자 합의 후 별도 커밋에서 수행한다. 담당자 양성훈, 추적은 이슈 [#225](https://github.com/team-youngkk/masit-on/issues/225).
- **추적 대상** — `MAX_CANDIDATES` 상한 인상은 여전히 미결이다. 손실 분석 9절에 남겼고 이 계약으로는 다장소 영상 중 후보 100건 초과분을 처리할 수 없다. 결정 주체는 김인안.
