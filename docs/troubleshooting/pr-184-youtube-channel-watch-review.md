...
| [구독 실패 상태·범위](https://github.com/team-youngkk/masit-on/pull/184#discussion_r3771627321), [외부 호출 순서](https://github.com/team-youngkk/masit-on/pull/184#discussion_r3771633348) | Hub 실패 시 상태·오류 범주 미기록, 외부 호출 전에 `UNKNOWN` 행이 커밋됨 | 애플리케이션 / 외부 연동 / 데이터베이스 | 수정 필요 | pending `UNKNOWN + 해시`를 Hub 호출 전에 저장. 명시적 4xx·5xx는 신규 행 삭제·기존 상태 복원, timeout/upstream은 `RENEWAL_FAILED + SUBSCRIPTION_*`로 기록 | 관리 서비스·PostgreSQL·WireMock 테스트 통과 |
| [202 callback 경쟁](https://github.com/team-youngkk/masit-on/pull/184) | Hub 202 응답 전 challenge가 도착하면 Token hash 저장 전이라 검증 실패할 수 있음 | 애플리케이션 / 외부 연동 | 수정 필요 | 외부 POST 전에 pending Watch를 커밋하고 지연 응답 중 challenge를 검증하는 경쟁 통합 테스트 추가 | `YoutubeChannelWatchActivationRaceIntegrationTest` 통과 |
| [RENEWAL_FAILED challenge](https://github.com/team-youngkk/masit-on/pull/184) | 실패 상태가 `enabled=true + hash`를 유지해 오래된 challenge로 ACTIVE 복귀 가능 | 애플리케이션 | 수정 필요 | `INACTIVE`·`RENEWAL_FAILED` challenge를 Token 일치 여부와 무관하게 403으로 거부 | `AiExtractionJobServiceTest` 통과 |
...
- 추가 관찰: 수정 전 신규 `setEnabled(true)`는 `subscription_token_hash`가 없는 `UNKNOWN` 행을 저장하고, `verifyChallenge`는 null 해시를 거부했다. 또한 후속 수정에서 이미 `ACTIVE`인 채널의 상태·해시를 새 요청으로 덮어쓰는 회귀와 외부 호출 전 `UNKNOWN` 행 커밋 순서 문제가 생겼다. 현재는 pending `UNKNOWN + 해시`를 Hub 요청 전에 저장해 즉시 challenge를 검증하고, 명시적 4xx·5xx는 보상하며 timeout/upstream은 `RENEWAL_FAILED`로 남긴다. 기존 `ACTIVE + 유효 해시`는 그대로 유지한다.

## 4. 근본 원인

활성화 UseCase가 `enabled=true`를 저장하면서 검증 Token을 생성하거나 해시를 저장하지 않아 `verifyChallenge`가 null 해시를 거부하는 단절이 있었다. 외부 구독을 연결한 뒤에는 DB 저장과 Hub 202 이후 callback의 순서를 계약으로 고정하지 않아, 저장 전 challenge와 실패 보상 경합이 남아 있었다. 또한 Webhook은 감시 행을 `FOR UPDATE`로 잠근 뒤 Job 생성까지 같은 트랜잭션을 유지하면서 DB statement timeout을 설정하지 않았다. Creator 오류는 여러 Application 서비스가 같은 메시지를 각자 생성했고, 이를 공통화한 팩토리를 Creator Application 루트에 두어 다른 도메인이 내부 구현을 직접 참조하게 되었다. `verifyChallenge`는 Token 해시만 확인하고 Watch의 `enabled` 상태만 확인해 `RENEWAL_FAILED`를 다시 ACTIVE로 만들 수 있었다. Redis 테스트는 애플리케이션 주입 Clock만 고정하고 Lua 스크립트가 사용하는 Redis `TIME`은 현재 시각으로 두었다.
...
- 이번 재리뷰 반영: 외부 Hub 구독을 `YoutubeChannelWatchSubscriptionPort`·`PubSubHubbubYoutubeChannelWatchSubscriptionAdapter`로 연결하고, 외부 호출 전에 pending `UNKNOWN + SHA-256 hash`를 커밋해 202 이전 challenge를 검증할 수 있게 했다. 명시적 4xx·5xx는 신규 pending 삭제·기존 상태 복원, timeout/upstream은 Token hash 조건부 `RENEWAL_FAILED` 기록으로 보상한다. `INACTIVE`·`RENEWAL_FAILED` challenge는 거부하고 ACTIVE 중복 활성화는 기존 상태·해시를 보존한다.
- 선택 이유: callback이 외부 POST 응답보다 먼저 도착할 수 있으므로 Token 검증 정보가 먼저 커밋되어야 한다. 동시에 명시적 실패는 핵심 데이터를 남기지 않고, 결과가 불명확한 timeout은 늦은 callback과 충돌하지 않는 재조정 상태로 남겨야 한다. 갱신·만료 스케줄러는 #180의 초기 활성화·Webhook 연결 완료 조건에는 포함하지 않고 P2-02 후속 범위로 남긴다.
...
| `./gradlew.bat test --tests "com.masiton.ai.application.YoutubeChannelWatchManagementServiceTest" --tests "com.masiton.ai.application.AiExtractionJobServiceTest" --tests "com.masiton.ai.infrastructure.persistence.JdbcYoutubeChannelWatchStoreIntegrationTest" --tests "com.masiton.ai.application.YoutubeChannelWatchActivationRaceIntegrationTest" --no-daemon --console=plain` | 통과 | pending 선행 커밋, 202 이전 challenge, 명시적 실패 보상, timeout 실패 상태, `RENEWAL_FAILED` challenge 차단 |
...
- 재발 방지: 외부 POST 전에 pending `UNKNOWN + hash`를 커밋하고, 202 응답 전 challenge를 발생시키는 WireMock·PostgreSQL 기반 경쟁 테스트를 추가했다. 명시적 실패 보상은 Token hash와 상태 조건부로 실행하며 `INACTIVE`·`RENEWAL_FAILED` challenge를 차단한다. Creator 도메인 외부 참조는 공개 `application.port.in` 경계로 고정했고, Redis 테스트는 서버 시계와 충돌하지 않는 미래 만료 시각을 사용한다. API 오류 계약 표도 같은 변경에서 갱신한다.
...
- 실제 YouTube 외부 구독 생성 Adapter는 `PubSubHubbubYoutubeChannelWatchSubscriptionAdapter`로 연결했다. Hub 요청 전에 pending `UNKNOWN + 해시`를 저장하고, challenge 성공 전 Webhook은 계속 차단한다. 명시적 4xx·5xx는 신규 Watch를 삭제하거나 기존 상태로 복원하고, timeout/upstream은 `RENEWAL_FAILED + SUBSCRIPTION_*`로 기록한다. 갱신·만료 스케줄러와 운영 Sandbox 검증은 P2-02 후속 운영 작업이다.
...
- 최신 P1 스레드 2건은 pending callback 경쟁과 `RENEWAL_FAILED` challenge 재활성화 문제로 분리해 반영했으며, 스레드 답글만 작성하고 해결 처리는 하지 않는다.
