...
|---|---|---|---|---|---|
| [r4012961603](https://github.com/team-youngkk/masit-on/pull/385#discussion_r4012961603) | 영상 상한에서 Cursor 보존 | 애플리케이션 | 이미 해결 | PR 기존 커밋이 남은 영상 수만 조회하고 `MAX_VIDEOS_PER_RUN`으로 다음 Cursor를 저장함 | 75개 상한·50개 처리 후 25개 조회 서비스 테스트 통과. DB 재검증은 Docker 기동 실패로 미완료 |
...
| [r4013054213](https://github.com/team-youngkk/masit-on/pull/385#discussion_r4013054213) | 호출 전 quota 예약·별도 백필 상한 | 인프라 | 이미 해결 | PR 기존 커밋에 Redis Lua 예약 및 두 API 각각 호출 전 차단이 있음 | Adapter의 quota 예약·차단 테스트 통과. Redis 재검증은 Docker 기동 실패로 미완료 |
...
| [r4015589465](https://github.com/team-youngkk/masit-on/pull/385#discussion_r4015589465) | 중간 실패·lease 상실 시 신규·재사용 누계 보존 | 데이터베이스 | 수정 필요 | Job 생성과 원장 기록을 `submitBackfillIfClaimActive`의 동일 트랜잭션으로 이동 | 컴파일·서비스 테스트 통과. 실제 DB 롤백·lease 재확보 회귀 테스트 추가, 실행은 환경 문제로 미완료 |
| [r4016300236](https://github.com/team-youngkk/masit-on/pull/385#discussion_r4016300236) | 페이지 상한도 재개 가능한 사유로 중지 | 애플리케이션 | 수정 필요 | `MAX_PAGES_PER_RUN`과 소유자·lease 조건부 중지, 같은 Cursor 재개, V17 전진 마이그레이션 추가 | 서비스 상한 테스트 통과. PostgreSQL 재개·수동 중지 구분·만료 소유자 거부 테스트 추가, 실행은 환경 문제로 미완료 |
...
| PostgreSQL·Redis·Flyway 포함 테스트 | 96건 중 40건이 Docker 초기화 실패 | 코드 assertion 결과로 해석하지 않음 |
...
| `JdbcYoutubeChannelBackfillRunStoreIntegrationTest`, `RedisYoutubeChannelBackfillQuotaIntegrationTest`, Flyway 통합 테스트 | 환경 실패 | Docker 엔진을 찾지 못해 DB·Redis 검증 미완료. 성공으로 간주하지 않음 |
...
9월 15일 기존 기록은 당시 60건 통과를 보고했다. 그 이력은 이번 코드의 DB 검증 증거로 대체 사용하지 않는다.
...
- 다음 확인: PR 작성자가 Docker 정상 환경 또는 PR CI에서 추가 통합 테스트를 실행하고 결과를 이 문서에 갱신한다. 추적은 #375·PR #385로 유지한다.
...
|---|---|---|---|---|
| 페이지 상한 후 외부 호출·재개 Cursor | 수동 중지로 첫 페이지 반복 가능 | 10페이지 상한 서비스 테스트·DB 재개 fixture | 외부 호출 0회 통과, DB Cursor는 미검증 | PR 작성자, Docker 복구 또는 PR CI, #375 |
| 원장 저장 실패 후 Job·누계 | 분리 커밋으로 Job만 남을 수 있음 | 원장 CHECK 실패 주입·Job 및 누계 조회 | 통합 테스트 추가, 결과 미검증 | PR 작성자, Docker 복구 또는 PR CI, #375 |
...
- 배포 API key 스레드는 [검증 답글](https://github.com/team-youngkk/masit-on/pull/385#discussion_r4023581418)을 게시하고 해결 처리했다. 자동 실행 스레드는 [결정 요청 답글](https://github.com/team-youngkk/masit-on/pull/385#discussion_r4023584284)을 게시하고 미해결로 유지했다. 최초 6개 중 해결 1개, 미해결 5개다.
- 페이지 상한·원자 저장의 PostgreSQL 검증과 quota Redis 재검증은 Docker 기동 오류 때문에 미완료다.
- 추가 수정 스레드는 PR 원격 반영과 통합 검증 성공을 확인한 뒤 해결 처리한다. 로컬 Docker 오류로 막힌 검증은 PR CI에서 이어서 확인한다.
