...
| 작성자 | jinyp01 |
| 처리 일자 | 2026-08-05 |
| 범위 | 리뷰 스레드 10건의 구현·검증과 후속 계약 결정 반영 |
...
| [상세 이름 maxLength 재확인](https://github.com/team-youngkk/masit-on/pull/135#discussion_r3717614254) | `maxLength`를 100에서 50으로 변경 | 애플리케이션 | 수정 불필요 | 100을 유지 | 위 스레드와 동일한 판단이다. `submitRename`이 전송 전 `collectionNameError`로 코드 포인트 50자 경계를 먼저 검증해 서버 예외가 유출되지 않으며, `maxLength={50}`은 보조 평면 문자 입력을 25자에서 잘라 계약을 어긴다. |
| [목록 이름 maxLength 재확인](https://github.com/team-youngkk/masit-on/pull/135#discussion_r3717614269) | `maxLength`를 100에서 50으로 변경 | 애플리케이션 | 수정 불필요 | 100을 유지 | 위와 동일하며 `submit`도 전송 전 `collectionNameError`를 검증한다. |
| [회원 잠금 예외 재확인](https://github.com/team-youngkk/masit-on/pull/135#discussion_r3717614274) | `member_account FOR UPDATE` 단건 조회가 없는 회원에 500을 반환하지 않도록 처리 | 데이터베이스 | 이미 해결 | 교차 도메인 경계 분리(e43f4a2)로 회원 활성 잠금이 이 파일에서 `MemberActiveLockService.lockActiveMember`로 완전히 이동했다. `JdbcMemberAccountRepository.findByIdForUpdate`는 `query(...).stream().findFirst()`로 빈 결과를 안전하게 `Optional.empty()`로 반환하고 `AUTHENTICATION_REQUIRED` 401로 변환한다 | `JdbcPersonalCollectionAdapter.java` 35행에는 더는 `member_account` 조회가 없음을 소스로 확인 |
| [이름 변경 Command의 교차 도메인 조회](https://github.com/team-youngkk/masit-on/pull/135#discussion_r3717629685) | `rename`이 쓰기 트랜잭션 안에서 교차 도메인 읽기 Projection(`queries.findSummary`)을 호출하지 않도록 분리 | 애플리케이션 | 수정 필요 | `rename`은 Personal 소유 `store.rename`만 수행하는 `void` Command로 좁히고, 응답 조립에 필요한 `restaurantCount`는 신규 `getSummary` 읽기 전용 메서드로 분리해 Presentation이 Command 완료 후 별도로 호출한다 | 신규 회귀 테스트 3건과 컨트롤러 순서 검증 테스트로 확인 |
...
조회를 orchestration Projection으로 옮긴 뒤에도 `rename`은 여전히 같은 `@Transactional` Command 메서드 안에서 응답용 `restaurantCount`를 채우려고 `queries.findSummary`를 호출했다. Command 완료 응답에 교차 도메인 필드가 필요하다는 요구와 "Command 경로에서는 읽기 모델 예외를 쓸 수 없다"는 규칙이 같은 메서드 안에서 충돌한 것이 원인이다.
...
| 재확인된 maxLength 스레드와 `collections-coordination.ts`/`PersonalCollectionService.normalizeName` 대조 | 두 계층 모두 코드 포인트 기준 50자이며 `submitRename`/`submit`이 전송 전 클라이언트 검증을 이미 수행 | 새 근거 없는 반복 요청으로 판단해 `수정 불필요` 재확인 |
| `JdbcPersonalCollectionAdapter.java` 35행 재확인 | 교차 도메인 분리(e43f4a2) 이후 이 파일에는 `member_account` 조회 자체가 없고 회원 잠금은 `MemberActiveLockService`로 이동 | `이미 해결`로 분류 |
| `PersonalCollectionService.rename` 트랜잭션 경계 점검 | `@Transactional` Command 메서드가 `orchestration.infrastructure.query` Projection(`queries.findSummary`)을 직접 호출 | `dependency-rules.md` 7절 위반으로 확인, Command와 조회를 분리 |

## 6. 최종 해결

- 변경 내용: 화면 상태 테스트와 누락 회원 오류 변환에 더해 `GET /api/me/collection-options`와 `AVAILABLE`, `ALREADY_INCLUDED`, `LIMIT_REACHED` 상태를 추가했다. 프론트는 상태별 표시·비활성화와 성공·실패 후 재조회를 수행한다. 후속 재리뷰에서는 Restaurant 상태가 필요한 목록·옵션·상세 조회를 Personal Query Port와 orchestration 읽기 Adapter로 옮기고, 회원 활성 잠금은 Member 공개 Port에 위임해 쓰기 Adapter에는 Personal 관계 잠금과 변경만 남겼다. `PersonalCollectionUseCase.rename`은 `store.rename`만 수행하는 `void` Command로 좁혔고, 응답 조립용 `restaurantCount`는 신규 `getSummary(memberId, collectionId)` 읽기 전용 메서드(`@Transactional(readOnly = true)`)로 옮겼다. `PersonalCollectionController.rename`이 Command 완료 후 `getSummary`를 별도로 호출해 같은 `PATCH` 응답 계약을 유지한다.
- 선택 이유: 기존 컬렉션 목록 계약을 보존하면서 단일 집계 조회에서 공개 수·실제 수·포함 여부를 분리하면 비공개 관계 수를 노출하지 않고도 정확한 상한 상태를 제공할 수 있다. 이름 변경은 `collection_restaurant` 관계를 바꾸지 않으므로 공개 수 조회를 Command 트랜잭션 밖에서 다시 수행해도 응답의 정확성에 영향이 없고, Command와 읽기 모델 예외 Projection을 분리하는 아키텍처 규칙을 지킬 수 있다.
- 변경 파일: 개인 컬렉션 API·추적 문서, `personal` Application/Presentation/JDBC 쓰기 Adapter와 Query Port, `orchestration.infrastructure.query` 읽기 Adapter와 경계 테스트, `frontend/lib/member/collections*`, `CollectionAddControl`과 옵션 렌더 테스트, `PersonalCollectionUseCase`/`PersonalCollectionService`/`PersonalCollectionController`의 `rename`·`getSummary` 분리와 관련 테스트.
- 고려한 대안: `maxLength={50}`은 유효한 보조 평면 문자 50개를 25개에서 잘라 제외해 채택하지 않았다. 기존 목록 응답 확장과 컬렉션 상세 반복 조회는 계약 오염, 페이지네이션과 비공개 관계 때문에 채택하지 않았다. `rename` 안에서 `this.getSummary(...)`를 호출해 같은 클래스 self-invocation으로 트랜잭션을 나누는 방식은 Spring AOP 프록시를 우회해 두 번째 호출이 첫 트랜잭션에 그대로 합류하므로 채택하지 않았다.
...
| `.\gradlew.bat compileJava compileTestJava` | 통과 | `rename`/`getSummary` 분리 뒤 컴파일 |
| `.\gradlew.bat test --tests "com.masiton.personal.*" --tests "com.masiton.architecture.PersonalPersistenceSqlBoundaryTest" --tests "com.masiton.orchestration.infrastructure.query.PersonalCollectionQueryAdapterIntegrationTest"` | 통과 | PostgreSQL Testcontainers를 포함한 personal 도메인 관련 테스트 58개. `rename`이 `queries.findSummary`를 호출하지 않음을 확인하는 신규 단위 테스트, `getSummary` 위임 테스트, Controller의 Command→조회 호출 순서 테스트를 포함한다 |
...
| Command 경로에서 읽기 모델 예외 Projection을 직접 호출하는 경로 | 1개(`rename`) | `PersonalCollectionServiceTest`의 Mockito 상호작용 검증 | 0개 | `rename`은 `store.rename`만 호출하고, `restaurantCount` 조회는 별도 `getSummary` 읽기 전용 메서드로 분리 | PR #135 작성자, 2026-08-05 |
...
- `PersonalCollectionUseCase.rename`이 `void`로 좁혀지면서 Command와 조회가 두 번의 별도 호출로 나뉘었다. 두 호출 사이의 극히 짧은 간격에 같은 컬렉션이 삭제되면 `getSummary`가 404로 실패할 수 있으나, 이는 이름 변경 자체가 이미 성공한 뒤의 동시 삭제 경쟁이며 기존에도 존재하던 최종 일관성 범위이므로 별도 처리를 추가하지 않았다.
