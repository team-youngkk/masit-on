...
| [GitHub Actions 백엔드 빌드·테스트](https://github.com/team-youngkk/masit-on/actions/runs/30963100332) | 통과 | 1차 리뷰 수정까지의 전체 자동화 검증. 옵션 API 변경은 푸시 후 최신 실행에서 재확인한다. |

## 8. 재발 방지 및 다음 확인

- 재발 방지: 화면 상태 5종, 없는 회원 생성, 옵션 상태 3종, 공개 수와 실제 수 분리, 성공·실패 후 재조회를 회귀 테스트로 고정했다.
- 다음 확인: 원격 브랜치 푸시 후 GitHub Actions와 리뷰어 재확인을 거쳐 해당 스레드를 해결한다.
...
| 컬렉션 추가 옵션 UI 자동 검증 | 0개 | 프론트 테스트 실행 | 8개 | 상태 표시·선택·비활성화와 성공·실패 후 재조회를 자동 검증 | PR #135 작성자, 최신 CI 완료 시 |

## 10. 남은 사항

- 최신 변경의 GitHub Actions와 리뷰어 재확인이 남아 있다.
