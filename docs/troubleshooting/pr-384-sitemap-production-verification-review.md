...
| 작성자 | w00lam |
| 처리 일자 | 2026-09-14 |
| 범위 | 운영 배포 경로, Googlebot robots 허용 여부, sitemap URL의 noindex 판정에 대한 미해결 리뷰 6건 |
| 주 문제 유형 | 배포 / 애플리케이션 |
...
- 실제 결과: 표준 배포가 끝나면 문서에 적힌 새 스크립트 경로가 없고, 기존 검사는 sitemap 선언 문자열·HTTP 200·`meta name="robots"`만 확인해 robots 차단, `meta name="googlebot"`, `X-Robots-Tag`를 놓친다.
...
둘째, 기존 smoke는 `robots.txt`의 sitemap 선언만 문자열로 확인하고 실제 robots 그룹을 해석하지 않았다. 따라서 `curl` 자체가 응답을 받을 수 있으면 Googlebot에게 허용되지 않은 URL도 성공으로 끝날 수 있었다. 응답 페이지 검사는 `meta name="robots"`만 대상으로 했고, 이미 저장한 response header 파일을 noindex 판정에 사용하지 않았다.
...
- 변경 내용: `HTMLParser`로 `meta name="robots"`와 `meta name="googlebot"`을 검사하고, 각 페이지의 response header에서 대소문자 무관 `X-Robots-Tag` `noindex`를 검사한다.
...
| `bash deploy/scripts/tests/sitemap-smoke-test.sh` | 통과 | Googlebot·wildcard robots 차단, Allow 우선순위, `googlebot` meta와 `X-Robots-Tag` noindex fixture 검증 |
...
- 로컬 코드와 문서 수정·검증까지 완료했지만, 저장소 지침에 따라 commit·push와 GitHub 인라인 답글·스레드 해결은 수행하지 않았다. 따라서 PR의 최초 미해결 스레드 6건은 원격에서 아직 미해결 상태이며, 변경 commit이 PR branch에 올라간 뒤 각 원문 스레드에 검증 결과를 답하고 해결해야 한다.
