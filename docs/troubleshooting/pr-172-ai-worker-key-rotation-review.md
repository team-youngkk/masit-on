...
| 작성자 | jinyp01 |
| 처리 일자 | 2026-08-11 |
| 범위 | 운영 활성화 설정, quota hard stop, 스케줄러 격리, 예외·로그, 임시 입력 키 교체, Worker·PostgreSQL 회귀 테스트 |
...
- 실제 결과: 운영 Worker 활성화 값이 컨테이너에 전달되지 않았고, quota 대기 작업은 `QUEUED`에 머물렀다. 전역 스케줄러가 장시간 점유될 수 있었으며, 키·DB 오류가 `INPUT` 종단 실패로 섞이고 예외 원인이 로그에서 사라졌다. 암호화와 Store 쓰기 정상 경로 일부는 테스트되지 않았다.
...
Worker 애플리케이션 설정을 추가하면서 운영 컨테이너의 SSM→호스트 환경→Docker 전달 경로와 전역 `@Scheduled` 실행 모델을 함께 추적하지 않았다. quota pre-check도 Provider 미호출만 확인해 관리자에게 노출되는 영속 작업 상태를 갱신하지 않았다.
...
- 변경 내용:
  ...
  - AI polling을 전용 스케줄러로 분리하고 heartbeat는 기존 전용 executor를 유지했다.
...
- 재발 방지: 운영 설정은 애플리케이션 속성뿐 아니라 SSM→실행 스크립트→컨테이너 전달을 계약 테스트로 고정했다. 키 교체, quota 상태, 처리기 부재, Store CTE 쓰기 경로를 회귀 테스트로 추가했다.
