...
| 작성자 | jinyp01 |
| 처리 일자 | 2026-08-11 |
| 범위 | 운영 Worker·Gemini 게이트 전달, quota hard stop, 기본·AI 스케줄러 격리, 예외·로그, 임시 입력 키 교체, Worker·PostgreSQL 회귀 테스트 |
...
| [Gemini 게이트 운영 전달](https://github.com/team-youngkk/masit-on/pull/172#discussion_r3755997596) | Worker 활성화 시 Provider가 항상 차단되는 운영 설정 누락 | 배포 | 수정 필요 | Gemini 활성화·Free Tier 확인·유료 결제 차단 값을 SSM에서 읽어 컨테이너에 전달 | 배포 스크립트 계약 테스트 통과 |
| [Gemini 게이트 운영 전달 중복 지적](https://github.com/team-youngkk/masit-on/pull/172#discussion_r3756006158) | Gemini Provider 활성화 게이트와 fail-closed 기본값 보장 | 배포 | 수정 필요 | 첫 번째 Gemini 게이트 수정으로 함께 처리하고 유료 결제 활성화 시 시작 차단 검증 | 배포·Provider 설정 테스트 통과 |
...
- 실제 결과: 운영 Worker 활성화 값을 보완한 뒤에도 Gemini 활성화·Free Tier 확인·유료 결제 차단 게이트가 컨테이너에 전달되지 않아 Provider가 항상 `PROVIDER_BLOCKED`로 실패했다. quota 대기 작업은 `QUEUED`에 머물렀고, AI 전용 `TaskScheduler`만 등록한 1차 수정은 이름을 지정하지 않은 예약 작업까지 AI 단일 스레드에 합칠 수 있었다. 키·DB 오류가 `INPUT` 종단 실패로 섞이고 예외 원인이 로그에서 사라졌으며, 암호화와 Store 쓰기 정상 경로 일부는 테스트되지 않았다.
...
Worker 애플리케이션 설정을 추가하면서 운영 컨테이너의 SSM→호스트 환경→Docker 전달 경로를 Worker 속성까지만 추적하고, 실제 외부 호출을 허용하는 Gemini의 별도 3중 게이트를 누락했다. 모든 기본값이 `false`인 fail-closed 구성이라 누락은 안전하게 호출을 막았지만 운영 활성화 경로도 함께 막았다. 또한 AI 전용 `TaskScheduler` 타입 빈을 추가하면 Spring Boot의 기본 `taskScheduler` 자동 구성이 조건부 후퇴한다는 컨텍스트 수준 상호작용을 리플렉션 테스트가 포착하지 못했다. quota pre-check도 Provider 미호출만 확인해 관리자에게 노출되는 영속 작업 상태를 갱신하지 않았다.
...
| `app-run.sh`와 Gemini 설정 바인딩 대조 | Worker 4개 값만 전달되고 Provider 게이트 3개는 기본 `false` 유지 | 동일 SSM→환경→Docker 경로에 Gemini 게이트를 추가하고 누락·유료 활성화를 테스트로 차단 |

## 6. 최종 해결

- 변경 내용:
  - Worker 운영 활성화·provider/application quota·window 값을 SSM에서 읽어 컨테이너에 전달했다.
  - Gemini 활성화·Free Tier 확인·유료 결제 상태를 SSM에서 읽어 컨테이너에 전달하되, 누락 시 모두 `false`이고 유료 결제 `true`는 애플리케이션 시작 전에 거부한다.
...
| `.\gradlew.bat test --tests "com.masiton.deployment.AppRunScriptContractTest" --tests "com.masiton.ai.infrastructure.provider.config.GeminiProviderPropertiesTest" --no-daemon --console=plain` | 통과 | Gemini 게이트 SSM 경로·Docker 전달·fail-closed 기본값과 유료 결제 활성화 거부 |
...
| 미해결 리뷰 스레드 | 14개 | GitHub review thread 재조회 | 0개 | 원인·변경·검증·기록 답글 후 14개 해결 | PR 작성자, PR #172 |
