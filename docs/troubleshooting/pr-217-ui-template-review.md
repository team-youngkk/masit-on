...
- 로컬 코드·문서 수정과 검증은 완료했다.
- 커밋·푸시는 사용자가 요청하지 않아 수행하지 않았다. 따라서 원격 PR의 최초 미해결 10개 스레드는 아직 답글·해결 처리하지 않았다.
- Node.js 24.18.0 지정 환경에서 CI 또는 재검증을 수행한 뒤, 각 원래 인라인 스레드에 원인·변경·검증·문서 링크를 답글로 남기고 해결 처리해야 한다.
