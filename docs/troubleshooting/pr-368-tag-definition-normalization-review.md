...
| 작성자 | 양성훈 (`@tjdgns0618`) |
| 처리 일자 | 2026-09-07 |
| 범위 | V9 legacy 태그 코드 전진 적용, Java/PostgreSQL 정규화 동등성, 원문·정규화 후 길이, AI 승인 태그 비교, 프론트 저장 안내 |
...
| [AI 신규 표시명 100자 상한](https://github.com/team-youngkk/masit-on/pull/368#discussion_r3949067147) | 101~200자 AI 라벨이 `AUTO_ACCEPT`된 뒤 `varchar(100)` INSERT에서 전체 등록을 롤백하는 문제 | 애플리케이션 | 수정 필요 | ADMIN·AI 원문 상한을 공통 100자로 고정하고 신규 후보 저장 전에 정책 거부 | 100/101자 정책·처리기 경계 테스트 통과 |
...
- AI 신규 태그 정책은 정규화 결과의 200자 상한만 검사해, 저장하는 표시명 원문이 `varchar(100)` 상한을 넘는 상태로 커밋 단계에 도달했다.
- 방문 태그 저장 안내는 아직 구현하지 않은 #364 동적 자연어 사전 반영을 약속했다.

## 4. 근본 원인

V10 계약을 설계할 때 새 작성 규칙만 검증하고 V9 작성자가 허용한 코드 집합을 실제 이전 버전 fixture로 대조하지 않았다. 정규화 알고리즘도 Java, PostgreSQL, AI 정책에 각각 구현해 locale, 공백, 길이 경계가 서로 달라졌다. 첫 리뷰 반영에서는 정규화 저장 키 200자만 공통화하고 AI가 실제로 저장하는 표시명 원문의 100자 물리 상한을 정책에 연결하지 않았다. 프론트 문구는 현재 이슈의 저장 경계를 후속 검색 사전 범위와 구분하지 못했다.
...
| AI 라벨 101자를 커밋 단계에서 예외 처리 | 같은 등록 단위의 Restaurant·Visit까지 롤백될 수 있음 | 후보 해석 단계에서 `TAG_POLICY`로 거부하고 유효한 등록은 계속 진행 |
...
- 공통 원문 상한을 100자로 정의하고 AI 신규 후보의 `rawLabel`과 `label`을 모두 저장 전에 검사한다. 101자 후보는 `AUTO_REJECT/TAG_POLICY`로 분류해 정식 태그 INSERT에 도달하지 않는다.
...
| 검증 | 결과 | 확인한 내용 |
|---|---|---|
| 관련 백엔드 테스트 6개 클래스 | 통과, 25건 | V9→V10, ADMIN·AI 원문·정규화 길이·원자성 |
| `.\gradlew.bat clean build --no-daemon --console=plain` | 통과, 1,465건 중 1,463건 통과·2건 조건부 제외 | 전체 백엔드 컴파일·테스트·계약 검사 |
...
- 원문 길이와 정규화된 저장 키 길이를 별도 계약으로 검사하고, 두 상한의 100/101·200/201 경계를 작성 경로마다 고정한다.
...
|---|---|---|---|---|---|
| 확인된 리뷰 결함 | 6건 | PR #368 미해결 리뷰 스레드 | 수정 후 회귀 테스트 실패 0건 | 6개 원인 모두 자동 검증 또는 빌드로 고정 | 양성훈, PR #368 리뷰 반영 시점 |
