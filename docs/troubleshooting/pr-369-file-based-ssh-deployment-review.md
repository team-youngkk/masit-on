...
| 작성자 | `w00lam` |
| 처리 일자 | 2026-09-07 |
| 범위 | Nginx signal/EXIT 복구, 파일 기반 TLS·앱 설정 셸 테스트의 필수 CI 연결, files 모드의 기존 ACM timer 비활성화·실패 복구와 완료 커밋 순서 |
| 주 문제 유형 | 배포 |
...
| [완료 커밋의 timer 상태 race](https://github.com/team-youngkk/masit-on/pull/369#discussion_r3949022460) | timer 상태 폐기보다 rollback 비활성화·trap 해제를 먼저 수행 | 배포 | 수정 필요 | 완료 경계를 `rollback 비활성화 → trap 해제 → 상태 폐기` 순서로 정렬하고 계약 테스트 추가 | `AppRunScriptContractTest`, `nginx-tls-files-test.sh`, deployment 계약 테스트 통과 |
...
- 실제 결과: 기존 스크립트는 `ERR` trap만 설치해 signal과 명시적 `EXIT` 경로의 복구를 보장하지 않았고, files 모드가 기존 ACM timer를 중지하지 않았다. 수정 후에도 완료 경계에서 timer 상태를 먼저 폐기하는 순서 race가 추가로 발견됐다. 신규 셸 테스트도 CI backend job에서 실행되지 않았다.
...
기존 `nginx-install.sh`의 rollback 경계가 `ERR`에만 걸려 있어 signal·EXIT를 같은 복구 함수로 전달하지 않았다. 또한 TLS source 분기에서 files 모드는 새 ACM timer 산출물 설치만 건너뛰고, 호스트에 이미 설치된 `masiton-tls-renew.timer`의 enabled/active 상태를 변경하지 않았다. 첫 수정의 완료 경계도 timer 상태를 rollback 비활성화·trap 해제보다 먼저 폐기해 좁은 signal race를 남겼다. 마지막으로 테스트 파일을 추가하면서 backend job에는 기존 Redis 셸 계약만 남아 있어, 새 파일 설정·TLS 계약이 CI 품질 게이트에 포함되지 않았다.
...
| 완료 경계의 상태 폐기·trap 해제 순서 대조 | rollback 활성 상태에서 `TIMER_STATE_CAPTURED=no`가 먼저 실행되는 race를 확인했다. | rollback 비활성화와 trap 해제를 먼저 수행한 뒤 상태를 폐기하고 순서 계약을 추가했다. |
...
- 변경 내용:
  ...
  - 정상 완료 경계를 `INSTALL_ROLLBACK_ACTIVE=no` → `trap - ERR EXIT INT TERM HUP` → `TIMER_STATE_CAPTURED=no` 순서로 고정해 복구 상태를 먼저 버리지 않도록 했다.
