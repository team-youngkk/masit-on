...
| 작성자 | inan0226 |
| 처리 일자 | 2026-08-12 |
| 범위 | AI 후보 검증·외부 참조 확인·대표 음식 카테고리 매핑·태그 정책·원자 커밋·방문 근거 판정·무주어 장소 대상·리뷰 회귀 테스트 |
...
| [#3762507415](https://github.com/team-youngkk/masit-on/pull/173#discussion_r3762507415) | 명시적 맛집 대상이 있는 무주어 방문 문장 허용 | 애플리케이션 | 수정 필요 | 1인칭 주어가 없어도 문장 시작 또는 장소 지시어 뒤에 검증된 맛집명이 방문 동사의 직접 대상이면 허용하고, 제3자·임의 선행 토큰은 계속 차단 | `VerifyAiContentCandidateServiceTest`의 `맛집을 방문했습니다`·`이 맛집에 다녀왔습니다` 및 제3자·영문 토큰 회귀 |
...
최신 미해결 지적의 원인은 방문 동사 앞 전체 문자열에서 `SUBJECT_PARTICLE`를 검색해 다절 1인칭 문장을 과차단한 것, 위치 조사만 확인해 검증되지 않은 일반 대상을 허용한 것, 방문 동사 뒤 부정 패턴만 사용해 `안/못 방문`을 놓친 것, 그리고 `Optional.empty()` 하나로 외부 기준정보 불일치와 방문 근거 부족을 표현한 것이다. 이번 4건은 `들렀` 어간과 영문 주어 토큰 경계가 허술하고, 검증된 맛집명이 부정·대조된 대상인지 확인하지 않으며, `PARTIAL` 상태 자체를 선택 태그 누락과 필수값 누락에 동일하게 적용한 문제였다. 추가로 방문 동사 직전 목적어를 제한하지 않아 앞 절의 맛집명 언급이 실제 방문 증거로 오인될 수 있었다. 이번 스레드의 직접 원인은 `hasFirsthandVisitContext()`가 검증된 장소 대상이 있어도 `FIRSTHAND_SUBJECT`를 항상 요구한 것이다. 보완 과정에서 무주어를 무제한 허용하면 `impostor 맛집 visited` 같은 임의 토큰도 통과할 수 있어, 장소 지시어·문장 시작으로 범위를 제한했다.

| 확인하거나 시도한 방법 | 결과 | 판단과 다음 단계 |
|---|---|---|
| PR #173 미해결 review thread 36건과 현재 head 대조 | 기존 35건 해결 후 무주어 방문 문장 지적 1건이 추가된 상태로 확인 | 장소 대상 허용 조건을 최소 수정하고 원문 스레드에 답글·해결 |
...
- 변경 내용:
  ...
  - 1인칭 주어가 없어도 문장 시작 또는 `이`·`그`·`저`·`해당` 뒤의 검증된 맛집명이 방문 동사의 직접 대상이면 허용하고, 제3자 주어·임의의 영문 선행 토큰은 계속 차단했다.
...
| `.\gradlew.bat test --tests com.masiton.orchestration.application.VerifyAiContentCandidateServiceTest --no-daemon --console=plain` | 통과 | 42 tests, 0 failures; `맛집을 방문했습니다`·`이 맛집에 다녀왔습니다` 허용과 제3자·임의 영문 선행 토큰 차단 |
...
- 다음 확인: 최신 무주어 방문 대상 스레드까지 inline 답글·해결 처리를 완료했다. 원격 전체 CI는 `workflow_dispatch` job 조건으로 실행되지 않았고 Docker/Testcontainers 통합 검증은 기존 성공 CI 기록과 로컬 환경 제약을 함께 보존한다.
...
| 추가 리뷰 미해결 스레드(8차) | 1건 | PR #173 스레드 API, 2026-08-12 | 0건 | 명시적 맛집 대상이 있는 무주어 방문 문장을 장소 지시어·문장 시작 조건으로 허용하고 제3자·임의 영문 토큰 회귀를 검증한 뒤 원문 답글·해결 | PR 작성자, PR #173 |
| 전체 리뷰 미해결 스레드 | 36건 | PR #173 스레드 API, 2026-08-12 | 0건 | 기존·추가 리뷰 36건에 원문 답글을 남기고 모두 해결 처리 | PR 작성자, PR #173 |
...
- 코드·문서 수정과 focused 테스트를 후속 커밋으로 원격 브랜치에 반영했고, 최신 리뷰 스레드에도 인라인 답글을 남겨 해결 처리했다. 원격 전체 CI는 workflow 조건상 실행되지 않았고 로컬 Docker Desktop 데몬 부재로 Testcontainers의 로컬 재실행도 환경 제약으로 남는다.
