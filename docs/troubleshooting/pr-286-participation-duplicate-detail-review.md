...
| [재리뷰 후속 P2 의견](https://github.com/team-youngkk/masit-on/pull/286) | 네 파일의 EOF 빈 줄로 `git diff --check`가 실패하고, predicate 반환값만 검증하므로 실제 지연 상세 조회 경합 회귀 테스트가 필요함 | 애플리케이션·품질 | 수정 필요 | EOF를 파일당 마지막 개행 하나로 정규화하고, 컴포넌트가 사용하는 비동기 상세 반영 경로를 지연 Promise로 검증하는 테스트를 추가했다. | `npm test`, `npm run typecheck`, `git diff --check` |
...
- 재리뷰 실제 문제: 기존 테스트는 `isCurrentParticipationDetailRequest`의 반환값만 확인해 지연된 상세 Promise가 완료된 뒤 `selected` 갱신을 막는 동작 자체는 검증하지 못했다.
- 재리뷰 기대 결과: 실제 화면 경로에서 사용하는 비동기 상세 반영 함수가 탭 전환 뒤 선택 상태를 갱신하지 않아야 한다.

## 4. 근본 원인

화면을 신규 접수(`view === 'new'`)와 내역(`view === 'history'`)으로 분리하면서 중복 오류 처리부는 기존 요청 상세를 `selected`에 저장하도록 남겨 두었지만, 상세 섹션의 렌더 조건을 `view === 'history' && selected`로 제한했다. 상태 저장과 표시 조건이 서로 다른 화면 정책을 사용한 것이 첫 원인이었다. 이후 상세 조회 토큰 보호가 목록에서 상세를 여는 경로에만 적용되고 중복 오류 경로에는 적용되지 않아, `switchTab`이 `selected`를 비운 뒤에도 오래된 Promise가 다시 상태를 채울 수 있는 비동기 경합이 남아 있었다. 기존 회귀 테스트도 이 경로의 최종 상태 변경이 아니라 predicate의 참·거짓만 확인하고 있었다.
...
| 네 변경 파일의 EOF 바이트와 `git diff --check`를 대조 | 마지막 개행 뒤 빈 줄이 남아 공백 오류가 발생함 | 마지막 개행 하나만 남기도록 정규화 |
| 컴포넌트가 사용하는 지연 상세 반영 경로를 Promise로 지연시켜 탭 전환 전후를 검증 | predicate만 호출한 기존 테스트로는 `selected` 미갱신을 보장하지 못함 | 지연 조회 완료 후 선택 목록이 비어 있는지 검증 |

## 6. 최종 해결

- 변경 내용: 선택된 상세는 `new`·`history` 화면 모두 렌더링하도록 수정했다. 중복 오류 응답에서 유효한 `requestId`를 추출하는 함수를 추가해 제보·신고 코드를 같은 검증 가능한 경로로 통합했다. 중복 상세 조회에는 요청 토큰을 발급하고 탭 전환 때 갱신되는 현재 `kind` ref와 함께 검증해 오래된 응답을 무시하도록 했다. 컴포넌트가 호출하는 비동기 상세 반영 함수를 분리해 지연 Promise 완료 뒤 stale 응답이 `selected`를 갱신하지 않는 회귀 테스트를 추가했다. 네 변경 파일은 마지막 개행 하나만 남도록 정규화했다.
...
| 검증 | 결과 | 확인한 내용 |
|---|---|---|
| `cd frontend; npm test` | 통과 | 291개 테스트 통과(기존 290개와 실제 지연 상세 경합 회귀 테스트) |
| `cd frontend; npm run typecheck` | 통과 | 화면·헬퍼 타입 검사 통과 |
| `git diff --check` | 통과 | 공백 오류 없음 확인 |

## 8. 재발 방지 및 다음 확인

- 재발 방지: `DUPLICATE_OPEN_SUBMISSION`과 `DUPLICATE_OPEN_REPORT`의 기존 요청 식별자 추출을 각각 테스트하고, 상세 렌더 조건을 `selected` 상태와 직접 연결했다. 중복 상세 조회는 요청 토큰과 탭 종류를 함께 확인하며, 지연 Promise가 완료된 뒤에도 stale 응답이 선택 상태를 바꾸지 않는 경계를 테스트한다. 변경 파일의 EOF는 `git diff --check`로 고정한다.
- 다음 확인: PR 브랜치 반영 후 리뷰어가 EOF와 실제 지연 상세 경합 회귀 테스트를 재확인한다.
