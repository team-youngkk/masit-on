...
이 디렉터리는 네 Workstream을 **누가 최종 책임지는지** 확정한다. 소유자는 담당 항목의 요구사항 구체화, 계약, 구현, 테스트, 문서화와 통합 완료를 책임진다.

원칙은 하나다. **각 Workstream과 기능 요구사항의 최종 책임자는 항상 한 명이다.** 공동 작업에서도 최종 병합 책임자는 한 명으로 유지한다.

현재 배정은 2026-07-27에 승인된 기준안이다. 일정 차단이나 지속적인 부담 불균형이 확인될 때만 [소유권 변경 절차](ownership.md#11-소유권-변경-절차)로 조정한다.
...
| 담당 | Workstream | 공통 최종 병합 |
|---|---|---|
| 양성훈 | WS-01 맛집 탐색 | 프론트엔드 공통 Layout |
| 박진영 | WS-02 맛집 상세·콘텐츠 | Flyway 마이그레이션 순서 |
| 이우람 | WS-03 유튜버 기반 탐색 | Spring Boot·Docker 실행 기반 |
| 김인안 | WS-04 관리자 인증·등록 | 인증 공통 설정 |
