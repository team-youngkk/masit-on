...
남은 차단 조건 다섯 중 넷은 **운영 환경에서 실행해야만 만들 수 있는 증거**다. AI Release holdout은 운영 Gemini 계정, Worker 자원 측정은 운영 EC2, Mobility quota는 운영 계정, 좌표 보강률은 운영 DB를 요구한다. 3차 확장 코드가 운영에 없으면 이 증거를 만들 수단 자체가 없고, 증거가 없으면 배포할 수 없다는 규칙이 걸려 있었다.
...
- 측정 중 Critical 결함이 확인되면 해당 기능 플래그를 즉시 `false`로 되돌린다. 플래그는 Parameter Store 값이므로 재배포 없이 컨테이너 재기동만으로 반영된다.
...
연기로 남는 위험을 명시한다. 제한 공개 기간 동안 검증 참여자는 품질이 확정되지 않은 AI 추출 결과와 코스 경로를 보게 된다. AI 후보는 관리자 검수를 거쳐 정식 등록되므로 잘못된 추출이 공개 데이터로 바로 넘어가지는 않지만, 검수 부하와 오탐은 측정 전까지 알 수 없다.
...
```text
코드 승격·운영 배포: GO (세 기능 공통)

제한 공개 활성화
  자연어 검색:    GO (증거 수집 목적)
  AI 영상 추출:   GO (증거 수집 목적, 관리자 검수 경유)
  맛집 코스 추천: GO (증거 수집 목적)
...
기능 활성화 상태는 Parameter Store 플래그가 권위 있는 기준이다(`/masiton/ai/gemini/enabled`, `/masiton/ai/worker/enabled`, `/masiton/integration/kakao-mobility/enabled`). 이 문서의 판정과 실제 플래그 값이 어긋나면 플래그를 먼저 맞추고 그 사실을 여기에 기록한다.
