...
P2 변경 전 배포본과 동일한 역사적 계약(`gemini-3.5-flash-lite`, Prompt `P1`, Schema `S1`, global endpoint)으로 두 영상을 직접 호출해 응답 구조를 확인했다. 이 결과는 P1 기준의 사전 실측 증거이며 P2 배포 검증 결과가 아니다. 응답 전문은 `ADR-AI-001` 5.1절에 따라 보존하지 않으며, 아래 집계와 최소 표본만 기록한다.
...
- **`MAX_CANDIDATES` 결정** — 상한을 100에서 300으로 올리고, 상한 때문에 후보를 잘라낸 응답은 그 사실을 결과에 표시한다. 근거와 상세는 아래 9.1절이다.
...
### 9.1 `MAX_CANDIDATES` 상한 결정

#### 재분석: 상한 초과의 실제 동작은 기각이 아니다

4.5절은 후보 수 상한을 Schema로 표현할 수 없어 시스템 지시 문장으로만 전달한다고 기록했다. 그 지시 문장은 현재 다음과 같다.

> `candidates`는 100개를 넘지 않아야 한다. 영상이 그보다 많은 장소를 다루면 근거가 가장 강한 후보만 남기고 나머지는 생략하라.

따라서 P2 이후 상한 초과의 정상 동작은 **응답 기각이 아니라 모델의 자체 절삭**이다. 실측 131건은 이 지시가 없던 P1 응답이다. `SCHEMA` 기각은 모델이 지시를 어겼을 때만 발생하는 예외 경로다.

이 사실이 문제의 성격을 바꾼다. 다장소 영상의 실제 위험은 "작업이 실패한다"가 아니라 **일부 맛집이 조용히 누락되고 관리자가 그 사실을 알 수 없다**는 것이다. 등록 단위별 독립 자동 등록을 도입해도 애초에 후보로 올라오지 않은 맛집은 등록되지 않는다.
...
|---|---|
| 상한 값 | 100 → **300** |
| 절삭 발생 시 | 결과에 절삭 사실을 표시하고 관리자 화면에 누락 경고를 노출한다 |
| 절삭 판단 주체 | 모델. 서버는 표시 값을 신뢰하지 않고 후보 수가 상한과 같으면 절삭 가능으로 함께 취급한다 |
| 300 초과 영상 | 여전히 절삭되지만 표시가 남으므로 관리자가 인지하고 수동 보완할 수 있다 |

300을 고른 근거는 실측 최대 131건에 대해 2배 이상 여유를 두면서, 후보 하나가 값·신뢰도·근거로 구성되는 것을 감안할 때 응답이 무료 tier 출력 한도 안에 머무르는 범위이기 때문이다. 실측 131건 응답은 이미 정상 처리됐다.

#### 계약 파급

이 결정은 **수신 계약과 시스템 지시를 함께 바꾸므로 버전 상향이 필요하다.** `BR-AIEXTRACT-004`가 버전별 재현성을 요구한다.

| 대상 | 변경 | 버전 영향 |
|---|---|---|
| 시스템 지시 문장 | 상한 숫자 변경, 절삭 시 표시 지시 추가 | Prompt `P7` → `P8` |
| 결과 Schema | 절삭 표시 필드 추가 | 결과 Schema `S1` → `S2` |
| 수신 검증기 | `MAX_CANDIDATES` 300, 절삭 표시 허용 필드 추가 | 계약 테스트 갱신 |
| `ai_candidate_snapshot` | 절삭 여부 컬럼 추가 | 새 Flyway 마이그레이션 |
| 관리자 상세 응답·화면 | 절삭 경고 노출 | API 계약·와이어프레임 |
