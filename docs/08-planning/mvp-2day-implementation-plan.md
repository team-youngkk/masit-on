...
- 2차 확장: 일반 사용자 인증, 찜, 최근 본 맛집, 지도, 유튜버 상세
- 3차 확장: 컬렉션, 인기 맛집, 큐레이션, 제보·신고, 알림
- 4차 확장: 자연어 검색, AI 영상 추출, 동선·코스 추천
...
9. Day 2 정오부터 기능 추가를 중단하고 결함 수정·회귀 검증만 수행한다.
...
| 시간대 | 양성훈 | 박진영 | 이우람 | 김인안 |
|---|---|---|---|---|
| Day 1 오전 | Next.js·공통 UI | Flyway·JPA 기반 | Spring Boot·Docker 기반 | 인증·WireMock 기반 |
| Day 1 오후 | WS-01 API·목록 UI | WS-02 API·상세 UI | WS-03 관계 조회 | WS-04 기본 등록 |
| Day 2 오전 전반 | 유튜버 필터 통합 준비 | 상세 콘텐츠 통합 준비 | Visit 등록 지원 | 인증·등록 완결 |
| Day 2 오전 후반 | 관리자 UI 지원 | Projection·쿼리 지원 | WS-01·02 관계 통합 | 관리자 UI API 연결 |
| Day 2 오후 | 통합·회귀·반응형 검증 | 통합·DB 정합성 검증 | 통합·실행 검증 | 통합·권한 검증 |
...
- Day 2 공동 작업은 표의 앞 담당자가 최종 병합하고 지원 담당자는 자기 소유 계약만 변경한다.
...
| 위치 | 대역 | 사용 목적 | 사용 시점 | 제거·전환 조건 |
|---|---|---|---|---|
| Kakao HTTP Adapter | WireMock Stub | 장소 정상·없음·timeout·제한·계약 오류 재현 | 전 개발·통합 테스트 | 최종 배포 전 실제 Sandbox 계약 테스트 추가 |
| YouTube HTTP Adapter | WireMock Stub | 채널·영상·게시 채널 검증과 오류 재현 | 전 개발·통합 테스트 | 최종 배포 전 실제 Sandbox 계약 테스트 추가 |
| WS-01 → WS-03 | 유효 맛집 ID Query Port Fake | Visit 구현 전에 유튜버 필터 조합 개발 | Day 1 | T-07 완료 후 실제 Query Adapter로 교체 |
| WS-02 → Visit 콘텐츠 | 상세 콘텐츠 Query Port Fake | 관계 데이터 없이 기본·빈·부분 실패 UI 개발 | Day 1 | T-07·T-09 완료 후 실제 Projection으로 교체 |
...
| 프론트엔드 | 고정 API 응답 Fixture | 백엔드 완성 전 목록·상세·관리자 상태 렌더링 | Day 1 | Day 2 오전 실제 API fetch로 교체 |

- WireMock은 실제 Kakao·YouTube 네트워크 호출을 대체하는 1차 MVP의 확정 검증 수단이므로 제거하지 않는다.
- Workstream 간 Fake와 프론트 고정 응답은 병렬 개발용 임시 대역이다. Day 2 통합 전에 실제 구현으로 교체한다.
...
### Phase 2 — Day 1 오후: Workstream별 핵심 수직 슬라이스
...
- Day 1 오전 병렬: `T-01`, `T-02`, `T-03`, `T-04`
- Day 1 오후 병렬: `T-05`, `T-06`, `T-07`, `T-08`
- Day 2 오전 순차·병렬: `T-09 → T-10/T-11/T-12`
- Day 2 오후 통합: `T-13 → T-14`
- 핵심 경로: `T-01 → T-03 → T-08 → T-09 → T-12 → T-13 → T-14`
- 첫 통합 시점: 맛집 등록 결과를 유튜버 조건 없는 목록에서 확인
- 두 번째 통합 시점: Visit 등록 결과를 유튜버 필터와 상세에 동시에 연결
- Day 2 정오 이후: 완료 정의에 없는 기능과 리팩터링 착수 금지
...
성능 전체 부하 시험은 2일 완료 조건에서 제외하지 않지만, 기준 데이터 자동 생성이나 도구 설치가 핵심 흐름을 지연시키면 쿼리 수·실행계획 Smoke Test까지만 수행하고 정식 p95 측정을 후속 안정화 위험으로 기록한다.
...
- AWS 배포: 2차부터 4차 확장 완료 후 [ADR-DEPLOY-001](../07-adr/platform/deploy-001-release-sequencing.md)에 따라 별도 수행한다.

## 12. 위험 요소

| 위험 | 가능성 | 영향 | 대응 |
|---|---:|---:|---|
| 신규 코드베이스 전체를 8인일에 구현 | 높음 | 높음 | 수직 슬라이스 병렬화, Day 2 정오 기능 동결 |
| T-08·T-09에 WS-04 작업 집중 | 높음 | 높음 | 이우람이 Visit, 박진영이 Projection 통합 지원 |
| 공통 계약 변경으로 병렬 작업 충돌 | 높음 | 높음 | Day 1 오전 DTO·Port 고정, 변경 시 네 명 즉시 공유 |
| 최신 고정 버전 설치 호환 문제 | 중간 | 높음 | T-01·T-02에서 최초 2시간 안에 설치 검증 |
...
| 테스트 실패 수정 시간이 없음 | 높음 | 높음 | Day 2 오후 신규 기능 금지, 핵심 경로만 수정 |

## 13. 가정 및 확인 결과

### 확정

- 구현 범위는 문서에 정의된 1차 MVP 전체다.
- 4명이 2026년 7월 27일부터 28일까지 모두 투입된다.
- 1차 MVP는 로컬 Docker 통합 검증으로 완료하며 AWS에 배포하지 않는다.
- AWS 배포는 2차부터 4차 확장 완료 후 수행한다.
- 제공된 와이어프레임은 1차 MVP 요소만 적용한다.
...
- 2일 동안 제품 범위와 API·데이터 계약을 추가 변경하지 않는다. 틀리면 모든 병렬 Task의 재작업이 발생한다.
