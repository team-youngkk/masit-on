...
> **2026-08-19 실측 정정.** 이 문단의 계산은 틀렸다. `UseContainerSupport`가 켜져 있어 `MaxRAMPercentage`의 기준은 호스트가 아니라 컨테이너 메모리 제한(`--memory 1024m`)이다. heap 상한은 하향 전후 모두 256 MB였고 backend 컨테이너 실사용은 398 MB였다. 아래 표의 JVM 항목과 합계는 성립하지 않는다. [전환 후 런타임 실측 기준선](post-cutover-runtime-baseline.md) 2.1절을 따른다.
...
- **CPU 크레딧.** t4g는 버스터블이다. small과 medium의 baseline 사용률·크레딧 적립률이 같은지 확인하지 않았다. AI Worker가 도는 인스턴스라 크레딧 소진은 실제 위험이다. → small의 적립률은 medium의 절반이고, 기동 직후 잉여 크레딧을 사용한 구간이 관측됐다(실측 2.3절).
