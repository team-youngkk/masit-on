...
| S2(ASG 2대) + E1 | 102% | 초과. 작업 요청자가 초과 비용을 부담하기로 승인 |
| S2 + E2 | 121% | 초과 |
| S1 + E3 | 114% | 초과 |
| S2 + E3 | 135% | 초과 |

**앱 인스턴스를 t4g.small로 내린다는 전제에서, S1과 Redis 분리는 E1 또는 E2로만 현재 환율 예산 안에 들어온다.** E2는 여유가 500원뿐이고 환율 최고치에서 초과하며, S2는 `ssm` 엔드포인트를 반영하면 E1에서도 예산을 넘는다. 이번 작업에서는 작업 요청자가 초과 비용을 부담하기로 승인했으므로 S2를 구현 대상으로 포함하되, 초과액과 실제 청구액을 운영 기록에 남긴다. Redis 접근 경로가 E3(NAT Gateway)로 결정되면 비용은 별도 승인 범위로 기록한다.
...
- **EC2 Instance Connect Endpoint 요금.** E1의 `$9.49`가 이 항목이 무료라는 전제에 달려 있다. AWS 사용 설명서의 생성·접속 페이지에 요금 문구가 없어 확인하지 못했다. **유료로 밝혀지면 E1을 다시 산정해야 하고, 현재 유일하게 여유가 있는 S1 + E1(81%)의 판정도 재확인해야 한다.**
