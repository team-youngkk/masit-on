...
| 검토 항목 | 결과 |
|---|---|
| 3항 비용 | **Redis 전용 인스턴스의 외부 접근 경로가 예산 안팎을 가른다.** 같은 구성이 경로에 따라 81%에서 114%까지 움직인다(5절) |
| 3항 운영 | Nginx는 ALB로 대체되지 않고 병존해야 한다(6.1절). **Blue-Green은 모든 Flyway 마이그레이션에 하위 호환을 강제한다**(6.2절). 무중단의 약한 고리가 Redis로 옮겨간다(6.3절) |
| 4항 일정 | 4차 기능 계획과 무관한 배포 전환 작업으로 분리해 산정한다. 기능 일정이 확정되지 않았으므로 기능 일정 지연으로 단정하지 않는다(7절) |
...
> **2026-08-19 실측 정정.** 이 문단의 계산은 틀렸다. `UseContainerSupport`가 켜져 있어 `MaxRAMPercentage`의 기준은 호스트가 아니라 컨테이너 메모리 제한(`--memory 1024m`)이다. heap 상한은 하향 전후 모두 256 MB였고 backend 컨테이너 실사용은 398 MB였다. 아래 표의 JVM 항목·합계와 그 표에서 파생한 **"여유 약 300 MB, medium보다 빠듯하다"**는 결론은 모두 성립하지 않는다. [전환 후 런타임 실측 기준선](post-cutover-runtime-baseline.md) 2.1절과 2절의 호스트 available 787 MB·backend 실사용 398.4 MiB를 기준선으로 삼되, 무부하 측정이므로 최대 부하 여유는 별도 부하 측정으로 확인해야 한다.
...
기존 표에 기반한 t4g.small의 "실사용 가능 메모리 약 1,900 MB, 여유 약 300 MB"라는 추정은 폐기한다. 현재 구성에서 확인된 무부하 기준선은 호스트 available 787 MB, backend 컨테이너 실사용 398.4 MiB이며, 이 값만으로 최대 부하 시 여유를 판정하지 않는다.
...
> **2026-08-19 실측 완료.** 세 항목 모두 [전환 후 런타임 실측 기준선](post-cutover-runtime-baseline.md)에서 수집했다. 하향이 이미 수행된 뒤의 사후 확인이다.

- **실제 RSS 측정.** `M2-09`가 미룬 항목이다. 4.1절 표는 전부 추정이며 실측으로 대체해야 한다. → 실측 2.절.
- **JVM heap 명시 고정.** 지금은 `MaxRAMPercentage` 기본값에 의존한다. 호스트 크기가 바뀌면 heap이 따라 움직이므로, 하향할 때 heap을 명시 옵션으로 고정할지 결정해야 한다. `Dockerfile` 변경이므로 소유자 합의가 필요하다. → **별도 조치 불필요.** 기준이 호스트가 아니라 컨테이너 제한이라 호스트 크기가 heap을 움직이지 않는다(실측 2.1절).
- **CPU 크레딧.** t4g는 버스터블이다. small과 medium은 모두 2 vCPU이고 인스턴스당 시간당 24 크레딧을 얻으므로 적립률 차이는 확인되지 않았다. AI Worker가 도는 인스턴스라 크레딧 소진은 여전히 실제 위험이다. → 기동 직후 잉여 크레딧을 사용한 구간이 관측됐고, 실측값은 24크레딧/시간 가설과 부합한다(실측 2.3절).
...
**셋째, Redis 기동 때 비밀값을 읽어야 한다.** 현재 구현은 [`redis-render-conf.sh`](../../deploy/scripts/redis-render-conf.sh)가 S3의 SSE-KMS 비밀 객체를 읽고, 기존 S3 gateway endpoint를 사용한다. Redis 부트스트랩에는 `ssm` interface endpoint가 필요하지 않으며, 앱의 Parameter Store 경로와 Redis의 bootstrap 경로는 분리한다([ADR-SEC-002](../07-adr/security/sec-002-redis-bootstrap-secret-transport.md)).

| 경로 | 이미지 | 관리 접속 | Redis 비밀값 접근 | 월 USD | 등급 |
|---|---|---|---|---:|---|
| **E1** | 이미지 tar를 S3에 두고 `docker load` (S3 게이트웨이 엔드포인트) | EC2 Instance Connect Endpoint | S3 SSE-KMS 객체·S3 게이트웨이 (`$0`) | **`$0`** | **EC2 Instance Connect Endpoint 요금 미확인** |
| **E2** | ECR 미러 + `ecr.api`·`ecr.dkr` 인터페이스 엔드포인트 + S3 게이트웨이 엔드포인트 | EC2 Instance Connect Endpoint | S3 SSE-KMS 객체·S3 게이트웨이 (`$0`) | `$18.98` | 인터페이스 단가는 M2 인용 |
| **E3** | NAT Gateway로 Docker Hub 직접 | SSM (NAT 경유) | S3 SSE-KMS 객체·NAT 경로 | `$43.07` | M2 산정이 쓴 전제 |

이번 기준선은 전용 Redis 인스턴스에 SSM Agent·CloudWatch Agent를 설치하지 않고, 관리 접속은 EC2 Instance Connect Endpoint로 수행한다고 가정한다. 현재 구현은 S3 SSE-KMS 객체와 S3 gateway endpoint로 Redis 비밀값을 읽으므로 Redis 전용 `ssm`·`ssmmessages`·`ec2messages` 엔드포인트를 요구하지 않는다. 앱 EC2의 SSM 운영 경로는 별도로 검증한다.

**E1의 EC2 Instance Connect Endpoint 요금은 아직 근거가 완결되지 않았다.** S3 게이트웨이 엔드포인트는 무료이고 Redis 비밀값에는 별도 interface endpoint가 필요하지 않지만, EICE 요금은 확인하지 못했다. 유료로 밝혀지면 E1을 다시 산정한다.
...
|---|---:|---:|---:|
| E1 | `$73.24` | 107,700원 | **72%** |
| E2 | `$92.22` | 135,600원 | **90%** |
...
| 접근 경로 | 월 USD | 월 KRW | 예산 대비 |
|---|---:|---:|---:|
| E1 | `$94.81` | 139,400원 | **93%** |
| E2 | `$113.79` | 167,300원 | **112%** ❌ |
...
**S2와 S1의 차이는 월 31,700원이다(E1 기준).** Redis 비밀값을 S3 gateway 경로로 옮겨 E1의 별도 interface endpoint 비용은 없어졌다. E2를 선택하면 ECR용 interface endpoint 2개의 비용만 추가된다.
...
|---|---:|---:|---|
| S1 + E1 | 107,700원 | 114,200원 | 없음 |
| S1 + E2 | 135,600원 | 143,800원 | 없음 |
| S2 + E1 | 139,400원 | 147,800원 | 없음 |
| S1 + E3 | 171,000원 | 181,300원 | 없음 (초과 유지) |

**S1 + E2는 1,470원 기준 90%, 최고 환율에서도 96%다.** S2 + E1도 최고 환율 기준 99%로 예산 안이다. EICE 요금과 실제 사용량·데이터 전송 비용은 별도 확인 대상으로 남는다.
...
- **ALB↔Nginx 구간 프로토콜.** HTTP target은 Nginx의 도메인 server가 HTTPS로 재리다이렉트해 loop가 생기므로, 구현안은 ACM 종단 뒤 ALB→Nginx를 HTTPS/443으로 재암호화한다. Nginx는 ALB subnet CIDR만 real IP proxy로 신뢰하고, backend readiness를 비민감 health alias로 반영한다.
...
**4차 기능 일정과의 비교값은 지금 산정할 수 없다.** `docs/08-planning`에 1·2·3차 확장 계획과 Task 분해는 있지만 **4차 확장 계획 문서가 없다.** 따라서 4차 기능 일정 지연으로 환산하지 않고, 배포 작업 자체의 기간과 기능 일정과의 병렬 가능 여부를 기록한다.

다만 이번 변경은 기능 확장이 아니라 배포 토폴로지 변경으로 진행한다. 따라서 4차 기능 계획을 착수 선행 조건으로 삼지 않고, 배포 작업 자체의 구현·검증 기간과 기능 개발에 대한 병렬 가능 여부를 별도로 기록한다.
...
**앱 인스턴스를 t4g.small로 내린다는 전제에서, S1과 S2 모두 Redis 비밀값을 S3 gateway로 읽는 E1 기준 현재 환율 예산 안에 들어온다.** E2는 S2에서 초과하므로 ECR interface endpoint 2개의 비용을 별도 승인 범위로 기록한다. Redis 접근 경로가 E3(NAT Gateway)로 결정되면 비용은 별도 승인 범위로 기록한다.
...
- **EC2 Instance Connect Endpoint 요금.** S3 gateway endpoint에는 추가 요금이 없고 Redis에는 SSM interface endpoint가 필요하지 않지만, EICE 요금은 확인하지 못했다. 유료로 밝혀지면 E1을 다시 산정한다.
