...
| `TST-E3-AI-005` | `BR-AIEXTRACT-001`, `BR-AIEXTRACT-009` | 다장소 영상 등록 단위 분해, Kakao 검색 WireMock, exact-name 우선 `NAME_AND_DISTRICT` 1건 자동 확정, exact 후보 부재 시 원본·기본 상호명 검색과 양방향 이름 containment/LIKE, Kakao 분류·AI 메뉴의 동일 단일 음식 카테고리 매핑·시구 일치·장소 동일성 판정 필수 필드·정확히 1건을 요구하는 fallback `NAME_CONTAINMENT_AND_DISTRICT_AND_CATEGORY`, 기존 `NAME_CONTAINS_AND_DISTRICT_AND_CATEGORY` 감사 JSON 조회 호환, 조건 불충족·0건 `PLACE_NOT_FOUND`, 복수 `PLACE_AMBIGUOUS`, `place_decision`·`category_decision`의 판정 근거·`matchedBy`, 일부 단위 차단 시 나머지 단위 등록 유지 | `E3-T06`, `E3-T07` |
...
장소명 매칭은 exact-name 우선 결과와 제한적 containment/LIKE fallback 결과를 `matchedBy`별로 분리해 품질을 평가한다. Release holdout 24건에서 자동 등록 정밀도·Critical 오연결 0건 등 [NFR-ACCURACY-002](../01-requirements/non-functional-requirements.md#nfr-accuracy-002-ai-추출-정확도재현율자동-등록-정밀도) 기준과 인간 판정 승인을 모두 충족하기 전에는 fallback을 포함한 장소 자동 확정을 전체 AI 자동 등록의 품질 게이트 통과로 판정하지 않는다. 현재 운영 정책상 장소명 완화 매칭 런타임 기본값은 `true`이며, 이 평가 게이트는 런타임 flag의 기본값이 아니라 전체 AI 자동 등록의 품질·확장 승인 여부를 판정한다.
...
- [ ] Release holdout 24건에서 exact-name 우선·제한적 containment/LIKE fallback의 `matchedBy`별 품질, 자동 등록 정밀도와 Critical 오류 0건 판정이 평가 보고서에 보존되고 품질 게이트 승인을 받는다.
