...
| `TST-E3-AI-005` | `BR-AIEXTRACT-001`, `BR-AIEXTRACT-009` | 다장소 영상 등록 단위 분해, Kakao 검색 WireMock, exact-name 우선 `NAME_AND_DISTRICT` 1건 자동 확정, exact 후보 부재 시에만 Kakao 분류·AI 메뉴의 동일 단일 음식 카테고리 매핑·시구 일치·장소 동일성 판정 필수 필드·정확히 1건을 요구하는 containment/LIKE fallback `NAME_CONTAINS_AND_DISTRICT_AND_CATEGORY`, 조건 불충족·0건 `PLACE_NOT_FOUND`, 복수 `PLACE_AMBIGUOUS`, `place_decision`·`category_decision`의 판정 근거·`matchedBy`, 일부 단위 차단 시 나머지 단위 등록 유지 | `E3-T06`, `E3-T07` |
