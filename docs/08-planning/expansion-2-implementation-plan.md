...
flowchart TD
    ...
    T10 --> T11
    T03 --> T13["E2-T13 전체 화면·사용자 여정 통합"]
    T05 --> T13
    T07 --> T13
    T11 --> T13
    T13 --> T14["E2-T14 보안·성능·운영·회귀"]
```
...
`E2-T10`은 알림 저장·목록·미읽음 수·읽음·보존을 독립적으로 준비한다. `E2-T11`은 `E2-T09`의 실제 상태 전이가 동작한 뒤 상태·이력·알림을 한 트랜잭션으로 연결한다. 기능 경로를 모두 마치면 `E2-T13` 브라우저 사용자 여정과 `E2-T14` 교차 품질 검증으로 종료한다.
