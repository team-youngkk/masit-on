...
워크플로 실행은 결과 요약을 artifact(`k6-normal-load-results`)로 14일 보관하고 job summary에도 남긴다. 직접 실행한 경우 `perf/k6/results/summary.json`과 `summary.txt`를 쓴다. 결과 파일 자체는 커밋하지 않고, 수치는 6절 표에 옮겨 적는다.
