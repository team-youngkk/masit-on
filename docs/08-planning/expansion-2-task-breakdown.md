...
2026-08-04 기준 회원·개인화 네 계약의 확정 상태 전환, 계약 전체 게이트 조건 2~6, M2 제한 공개에서의 회원·관리자 흐름과 기존 복구 리허설 근거를 충족했다. 검증 참여자 쿠키 접근은 역사적 기준선으로 보존하며, 현재 공개 전환은 [ADR-DEPLOY-006](../07-adr/platform/deploy-006-public-release-without-validation-gate.md)에 따라 gate 없이 수행한다. 운영 지도에서도 새로고침 직후 `map-points` 요청 1건을 기준으로 이동·확대·축소 뒤 요청 수가 1건 그대로 유지되고 결과·선택 상태가 유지됨을 확인해 `implementation_gate`를 `Ready`로 전환한다.
...
|---|---|---|---|---|---|
| `E2-T01` | 1차 확장·운영 품질 기준선을 확인한다 | 이우람 / 김인안 | E1 완료 또는 명시적 선행 | 제한 | 회원·인증·찜, `E1-T11`~`E1-T12`, M2 운영 선행과 `E1-T13` 역사 증거 대조, 현재 ADR-DEPLOY-006 gate 제거·운영 경계 회귀, ClassNotFoundException 품질 게이트, CI 통과 |
...
| `E2-T13` | 컬렉션·인기·제보·신고 화면 간 진입점과 공통 상태 계약을 정합화한다 | 전원 / 상호 교차 리뷰 | `E2-T03`, `E2-T05`, `E2-T07`, `E2-T11` | 불가 | 화면 연결, 정상·빈·오류·인증 만료 상태 정합화 (PR #143) |
| `E2-T14` | 알림 화면을 연결하고 지원 브라우저·360px·키보드 접근성을 포함한 전체 사용자 여정과 E2E 자동화를 완료한다 | 전원 / 상호 교차 리뷰 | `E2-T13` | 불가 | `TST-E2-NOT-001`, `TST-E2-E2E-001`; 탐색·개인화·참여·알림 브라우저 여정 검증 |
| `E2-T15` | 보안·성능·운영·회귀 검증을 완료한다 | 전원 / 상호 교차 리뷰 | `E2-T14` | 불가 | 전체 FR 추적, `TST-E2-SEC-001`, `TST-E2-PERF-001`, CI·운영 기준 통과 |
...
- 2026-08-04 운영 브라우저에서 M2 검증 참여자 쿠키로 진입한 뒤 회원 찜·최근 본 목록과 관리자 맛집·유튜버·영상·Visit 네 등록 화면이 모두 렌더링됨을 직접 확인했다. 이는 역사적 제한 공개 증거이며, 정식 공개 전환에서는 검증 쿠키 없이 같은 회원·관리자 Bearer 흐름과 공개 화면·API를 확인한다. 지도 이동·확대·축소 전후 `map-points` 요청 1건 유지는 사용자가 확인했다.
...
### E2-T13~E2-T15 통합과 종료

- `E2-T13`은 컬렉션·인기·제보·신고 화면의 진입점과 정상·빈·오류·인증 만료 공통 상태 계약을 정합화한다(PR #143, 이슈 #116).
- `E2-T14`는 서비스 내 알림 화면을 연결하고, 지원 브라우저·360px·키보드 접근성을 포함한 전체 사용자 여정과 `TST-E2-NOT-001`(알림함·`99+` 브라우저 표시), `TST-E2-E2E-001`을 완료한다(이슈 #144). E2E 자동화 도구 도입은 3차 확장에서 별도 ADR로 결정하며, 도입 전까지는 수동 검증으로 대체한다.
- `E2-T15`는 V3 migration, 권한·동시성·보존·실행계획·부하·운영 지표·전체 회귀와 네 추적표를 최종 판정한다(이슈 #117).
