...
이 문서는 개인 컬렉션, 인기 맛집, 큐레이션, 사용자 제보·신고, 사용자 알림의 2차 확장 계약을 작성하기 전에 현재 저장소의 선행 상태를 확인한다. 문서화는 선행 기능의 구현 완료와 무관하게 진행할 수 있지만, 구현 Task는 이 문서의 착수 게이트를 통과한 뒤에만 분해한다.
...
2차 확장 문서화는 지금 시작할 수 있다. 구현 Task 분해와 담당자 배정은 다음 조건을 모두 충족한 뒤 시작한다.
