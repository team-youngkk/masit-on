...
| `E1-T12` 가입 이메일 인증 8자 코드가 구현됐는가 | **구현 근거 있음** | 가입·재발송은 허용 문자 32자의 8자 CSPRNG 코드를 발급하고, API·화면은 입력 정규화와 요청 출처당 10분 10회 제출 제한을 적용한다. 비밀번호 재설정 Token 형식과 암호화 Outbox는 유지하며, 최종 충족 판정은 구현 PR의 회원 인증 회귀와 리뷰 통과 뒤 확정한다. |
