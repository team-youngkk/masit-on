...
| `E1-T11` 지도 뷰포트 비종속 조회가 구현됐는가 | **미충족(구현 PR 검증 중)** | 양성훈 owner 승인으로 문서 계약은 필터 기반 조회와 지도 이동 시 결과 유지로 변경됐다. Controller·Command·SQL·TanStack Query `queryKey`와 지도 `idle` 재조회에서 bounds를 제거한 구현 PR이 제출됐으나 아직 `develop`에 병합되지 않았고, 브라우저 회귀와 `E1-T10` 재검증도 남아 있다. 병합과 그 재검증 전까지 2차 확장 기준선은 통과하지 않는다. |
