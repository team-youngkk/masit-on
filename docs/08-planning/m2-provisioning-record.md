...
`dependencies` 그룹에 포함되므로 연결·인증이 성립하지 않으면 새 이미지 배포의 ready
확인이 실패한다. 이 경우 배포 워크플로가 실패 상태로 끝나며 기록된 직전 이미지 SHA로
같은 승인 게이트의 롤백 절차를 실행한다.
...
유효 Origin의 refresh/logout이 403이 아닌 인증 실패로 진입하는지와 서로 다른 전달 IP가
하나의 rate-limit 버킷으로 합쳐지지 않는지를 확인한다.
...
**이 경로는 이 PR이 병합된 뒤에 쓸 수 있다.** `workflow_dispatch`는 워크플로 파일이 **기본 브랜치에 있어야** 발동한다. 이 저장소의 기본 브랜치는 `main`이고 현재 `main`에는 `.github/workflows/ci.yml`이 없다. `deploy/m2` → `main` 병합이 곧 그 파일을 기본 브랜치에 올리므로 병합과 함께 해소된다. 6.6절의 `workflow_run` 문제와 같은 계열의 제약이다.

그래서 워크플로 구성과 입력 검증까지만 확인했고 실제 `workflow_dispatch` 롤백 배포는 17절에 미검증으로 남긴다.
