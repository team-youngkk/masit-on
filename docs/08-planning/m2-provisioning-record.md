...
**이 계정은 맛잇온 전용이 아니다.** 2026-06-05에 만든 다른 프로젝트의 ECR 리포지토리 `commerce-payment`(이미지 8개)가 있고 삭제된 RDS의 로그 그룹 `RDSOSMetrics`가 남아 있다. 예산 범위 영향은 13절에 적었다.
...
| 컨테이너 기동 | 미확인 (14절) | Next.js 16.2.11 기동, `/` → `200` |
...
- **팀원 관리자 계정 4개**(`tjdgns0618`·`rladlsdks12`·`jik123456789`·`wlam135`). 비밀번호를 운영자가 Parameter Store에 등록하면 [`admin-account-create.sh`](../../deploy/scripts/admin-account-create.sh)로 생성하고, **생성 후 password 파라미터를 삭제한다.**
- **`masiton-admin` 삭제.** 배포 검증용으로 만든 계정이며 팀원 계정 로그인 확인 후 폐기한다.
- **로그인 실패 5회 차단.** 카운터의 `source`가 연결 원격 주소 기준이고 Nginx 경유 요청이 모두 `127.0.0.1`로 보여 시험하면 15분간 모든 로그인이 막힌다. `M2-13` 진입 직전에 한다.
- **WS 담당자별 인수 확인.** 계획 `M2-12`는 4개 WS 담당자가 각자 흐름을 확인하고 기록하도록 정했다. 위 검증은 이우람이 API 수준에서 수행한 것이며 화면 수준 확인은 담당자별로 남았다.
...
- **로그인 실패 제한.** 실패 카운터의 `source`가 연결 원격 주소 기준이고 Nginx 경유 요청이 모두 `127.0.0.1`로 보여, 5회 실패를 만들면 15분 동안 모든 출처의 로그인이 막힌다. `M2-13` 진입 직전에 확인한다(12.5절).
- **팀원 관리자 계정과 `masiton-admin` 폐기.** 12.5절.
