...
| 외부 관측성 | `$0` | 2026-09-03 CloudWatch·알람·Slack 경로 폐기 |
...
M2에서는 Redis를 앱 EC2에 동거시키므로(5.4절) 남는 변수는 RDS 클래스뿐이다. 현재 공통 기반은 EC2 t4g.medium `$30.37` + EBS 30 GiB `$2.74` + Elastic IP `$3.65` + RDS gp3 20 GiB `$2.62` + Route 53 `$0.50` + ECR `$0.20` = **`$40.07`**이다. 배포 고도화 private Redis 비용은 별도 영향 검토의 S1/S2 산정에 포함한다.

| 구성 | RDS | RDS 요금 | 월 USD | 월 KRW | 예산 대비 |
|---|---|---:|---:|---:|---:|
| **A** (채택) | db.t4g.micro | `$18.25` | **`$58.32`** | **85,700원** | **57%** |
| **B** (필요 시 승급) | db.t4g.small | `$37.23` | `$77.30` | 113,600원 | 76% |
...
- **채택 구성 A의 월 예상 비용은 `$58.32` = 약 85,700원으로 예산 목표의 57%다.** `M2-01` 완료 조건 "산정한 월 예상 비용이 예산 목표 이내다"를 만족한다.
- **RDS를 db.t4g.small로 올려도 약 113,600원(76%)으로 여유가 남는다.** `M2-12`에서 메모리·CPU 크레딧이 부족하면 승급할 수 있다.
- **환율이 2026년 최고치 1,559원으로 가도 구성 A는 약 90,900원(61%)이다.** 환율 변동이 판정을 뒤집지 않는다.
- **NAT Gateway와 인터페이스 VPC 엔드포인트를 M2 구성에 넣지 않는다.** 6.1절 제외 표에서 보듯 두 항목만으로 예산의 42%·28%를 쓴다. 앱 EC2는 퍼블릭 서브넷에 두고 Elastic IP로 Parameter Store·ECR에 접근하며, 인바운드는 보안 그룹으로 80·443과 작업자 IP의 22만 허용한다(`M2-03`).
...
- **기존 CloudWatch 비용 산정은 폐기됐다.** 2026-09-03 CloudWatch·알람·Slack 경로를 운영 배포에서 제거했으며, 현재 비용은 실제 AWS 청구와 대조한다.
