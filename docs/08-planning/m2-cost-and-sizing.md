...
**선행 절차는 처리됐다.** 이 결정은 [ADR-DATA-005](../07-adr/data/data-005-redis-refresh-token.md) 6절의 "운영은 사설 서브넷 전용 Redis 8.8 인스턴스를 사용한다"는 표현과 어긋났다. 강제 규칙(8.8 계열, 인터넷 미노출, 네임스페이스, TTL, 회전 원자성)은 모두 유지되고 배치 표현만 어긋났으므로, **2026-07-30 공동 owner(김인안·이우람) 합의로 6절의 배치 표현만 개정했다.** loopback 바인딩은 "인터넷 미노출"을 사설 서브넷보다 강하게 만족하므로 10·11절은 손대지 않았다.
