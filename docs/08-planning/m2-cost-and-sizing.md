...
Redis를 앱 EC2에 동거시키기로 했으므로(5.4절) 남는 변수는 RDS 클래스뿐이다. 공통 기반은 EC2 t4g.medium `$30.37` + EBS 30 GiB `$2.74` + Elastic IP `$3.65` + RDS gp3 20 GiB `$2.62` + Route 53 `$0.50` + ECR `$0.20` + CloudWatch `$3.00` = **`$43.07`**이다.
...
Redis는 앱 EC2 안에서 돌아 인스턴스·스토리지 요금이 추가되지 않는다. 도메인 요금은 `M2-02`(#41)에서 도메인이 정해지지 않아 제외했다. `.com` 기준 연 `$14`로 월 환산 약 1,700원이며 판정을 바꾸지 않는다.
...
| IAM 역할의 결제 정보 액세스 | 필요 | 루트 로그인으로 활성화. 켜지 않으면 예산 생성이 차단된다 |
| 월 예산·초과 알림 | **미완** | 아래 조건대로 생성한다 |
...
예산은 **자원 생성보다 먼저** 설정한다(계획 9절 첫 번째 위험). 완료 조건이 "예산 초과 알림이 **실제로** 발송되며"이므로 임계값을 낮춘 별도 시험 예산으로 알림 도달을 한 번 확인한 뒤 그 시험 예산만 삭제한다. 본 예산의 임계값을 낮췄다 되돌리는 방식은 되돌리기를 잊으면 알림이 계속 발송되므로 쓰지 않는다.

**AWS Budgets는 하루 약 3회만 평가한다.** 시험 알림이 즉시 도달하지 않으므로 예산을 먼저 만들어두고 대기 중 다른 Task를 진행한다.
