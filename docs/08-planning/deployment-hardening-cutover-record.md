...
[codedeploy.tf](../../infra/production/terraform/codedeploy.tf)가 이 상황을 위해 `deployment_alarms_enabled`를 두고 있다. 최초 seeding에서만 `initial_alarm_seeding=true`와 `deployment_alarms_enabled=false`를 같은 명령행에 넘겨 한 사이클을 완화하고, 지표 발생과 알람 `OK` 전환을 확인한 뒤 곧바로 `initial_alarm_seeding=false`·`deployment_alarms_enabled=true`로 되돌린다. `terraform.tfvars`는 수정하지 않는다. Redis 복구 모드에서는 이 완화를 사용하지 않는다.
