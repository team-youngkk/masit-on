...
- 테이블·ADR: V3 `favorite`, `recent_restaurant_view`; [ADR-DATA-003](../07-adr/data/data-003-spring-data-jpa.md), [ADR-AUTH-002](../07-adr/security/auth-002-member-jwt-refresh-token.md)
- 수정 예상 영역: 신규 개인화 도메인/Port, Flyway V3, 맛집 상세 성공 시 최근 기록 부수효과, 본인 자원 인가·탈퇴 정리
- 선행 / 병렬 / 크기: `E1-T03` / `E1-T04`와 가능 / L
- 테스트 범위: V1→V3, 중복·동시 찜, `GREATEST` upsert, 최신 50건·30일 보존, 비공개 맛집 숨김, 다른 회원 접근·탈퇴 정리
