...
이 디렉터리는 맛잇온이 **무엇을 하는 서비스이고 1차 MVP에서 어디까지 만드는지**를 확정한다. 이후의 요구사항, PRD, API 계약, 데이터 모델과 ADR은 모두 여기서 정한 범위와 용어를 전제로 작성된다.
...
2. [범위](scope.md): 1차 MVP 포함·제외 범위, 확장 단계, 범위 경계 규칙과 변경 절차
...
| `scope.md` | 1차 MVP에 무엇이 들어가고 무엇이 빠지는가? | 기능의 구체적 결과와 판단 기준 |
...
- 와이어프레임에는 지도·테마·보관함 같은 확장 기능이 그려져 있다. [제외 범위](scope.md#4-1차-mvp-제외-범위)에 있으면 화면과 Route를 만들지 않는다.
- 1차 MVP는 로컬 Docker 통합 검증까지이며 AWS 운영 배포를 포함하지 않는다.
