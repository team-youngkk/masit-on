...
- 맛집 평점·리뷰·영업시간은 확정 데이터 모델에 없으므로 표시하지 않는다.
- 맛집 카드와 상세의 대표 이미지는 공개·유효한 관련 영상의 기존 YouTube 썸네일을 사용한다. 후보가 없거나 로드에 실패하면 카테고리 placeholder를 표시하며, 별도 이미지 저장이나 외부 이미지 API 호출은 하지 않는다.
...
- [ ] 맛집 카드와 상세에 평점·리뷰·영업시간 등 미지원 데이터가 없다.
- [ ] 대표 이미지는 관련 영상 썸네일을 우선 사용하고, 없거나 로드에 실패하면 카테고리 placeholder로 fallback한다.
