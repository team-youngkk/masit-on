...
- 입력 검증 실패: 제출 전 필드 오류를 표시하고 안전한 입력값을 유지한다. 보충 입력 `CONFIRM`이 후속 장소·카테고리·YouTube·방문·중복 검증에서 실패하면 기존 차단 사유와 이번 검증 실패 사유를 구분해 표시하고, 등록 단위 상태·정식 Entity·새 AI 작업은 만들지 않는다.
