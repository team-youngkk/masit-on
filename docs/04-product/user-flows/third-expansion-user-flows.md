...
4. 관리자는 차단 사유를 확인한 뒤 사유별로 정해진 경로만 따른다. 장소 동일성·카테고리 예외는 보조 입력 화면에서 `CONFIRM`으로 등록하고, 필수 필드·방문 근거 부족은 재추출 또는 기존 수동 등록으로, 외부 서비스 오류는 등록 재실행 또는 기존 수동 등록으로 처리한다. 보충 `CONFIRM` 후 외부 서비스 오류가 나면 등록 재실행은 화면에 남아 있는 같은 보충값·사유를 포함한 `CONFIRM` 재전송이다. 업무 중복은 기존 등록 결과 확인(`EXISTING_RESOURCE`)으로만 처리한다. 어느 사유든 `DISCARD`로 폐기할 수 있다. 정상 결과를 승인하기 위한 확정 버튼은 제공하지 않는다.
5. 부분 결과를 완전 결과와 같은 성공 지표로 집계하지 않는다.

### 3.5 오류·재시도·복구

- 입력 검증 실패: 제출 전 필드 오류를 표시하고 안전한 입력값을 유지한다. 보충 입력 `CONFIRM`이 후속 장소·카테고리·YouTube·방문·중복 검증에서 실패하면 기존 차단 사유와 이번 검증 실패 사유를 구분해 표시하고, 이번 실패 사유에 맞는 복구 경로만 노출하며, 등록 단위 상태·정식 Entity·새 AI 작업은 만들지 않는다.
...
1. 사용자가 공개 맛집 검색 결과에서 “코스에 추가”를 선택하거나, 로그인 후 코스 화면의 “내 찜에서 찾기”에서 찜 목록을 불러온다.
2. 찜 목록은 최신 찜 순으로 페이지를 제공하고 `더 보기`로 다음 페이지를 이어 붙인다. 검색 결과와 찜 목록에 같은 맛집이 있으면 식별자 기준으로 이미 선택된 상태를 표시한다.
3. 로그인하지 않은 사용자는 기존 검색 기반 코스 선택을 계속 사용할 수 있고, 찜 후보 영역에서는 로그인 안내를 본다. 찜 목록이 비어 있으면 오류가 아닌 정상 빈 상태로 맛집 탐색을 안내한다.
4. `COURSE-BUILDER`는 두 후보 원천의 선택을 하나의 목록으로 합쳐 선택 순서, 공개 상태와 좌표 가능 여부를 표시한다.
5. 1개 이하이면 최소 2개 안내, 5개이면 추가 차단 상태를 표시한다.
6. 첫 맛집은 출발점으로 고정되며 사용자가 순서를 바꿔 첫 항목을 바꾸면 새 첫 항목이 출발점이 된다.
7. 찜 목록 조회가 실패해도 이미 선택한 후보와 계산 결과는 지우지 않고 재시도 안내를 표시한다. 좌표 없음·비공개·삭제 맛집이 있으면 조용히 제외하지 않고 문제 항목을 표시해 교체를 요구한다.

### 4.2 경로 계산과 정상 결과

1. 유효한 2~5개 후보에서 사용자가 계산을 요청한다.
2. 화면은 선택 목록을 유지하고 중복 제출을 막는다.
3. 시스템은 첫 맛집을 고정한 자동차 방문 순서와 실제 구간 경로 형상을 계산한다.
4. 실제 경로 합계가 30km 이하이면 `COURSE-RESULT`에 제안 순서·구간별 거리/시간·전체 거리·생성/만료 시각을 표시하고, 지도에는 방문 순서 마커와 구간 형상을 이어 그린 실제 경로 선을 표시한다.
5. 사용자는 지도 마커 또는 텍스트 순서 목록에서 항목을 선택할 수 있고, 선택은 양방향으로 동기화된다. 최초 표시와 새 경로 조회 때 전체 경로가 보이도록 지도 영역을 맞춘다.
6. 사용자는 맛집 상세로 이동하거나 선택을 수정하거나 새 경로를 조회한다.
7. 만료 뒤 결과를 열면 오래된 거리·시간과 지도 경로를 최신으로 표시하지 않고 재조회를 요구한다.

### 4.3 형상 저하와 SDK 실패

1. 구간별 거리·시간 계산은 모두 성공했지만 일부 구간의 실제 경로 형상만 제공되지 않으면 `COURSE-RESULT`는 그대로 순서·거리·시간을 표시하고 지도에서 해당 구간만 형상 없음으로 표시한다.
2. Maps SDK 로딩 실패 또는 API 키 누락이면 지도 대신 오류·재시도 안내를 표시하되 순서·거리·시간 텍스트 목록은 완전하게 유지한다.
3. 지도 이동·확대·축소는 서버 재조회나 코스 재계산 조건으로 사용하지 않는다.

### 4.4 거리 초과와 외부 실패

- 30km 초과: 경로를 결과로 제공하지 않고 어떤 후보를 제거·교체할지 안내한다.
- timeout·429·제공자 장애: 거리·시간과 지도를 표시하지 않고 입력 목록·입력 순서·오류 범주만 유지한다.
