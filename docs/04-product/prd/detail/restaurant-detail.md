...
맛집 기본 정보와 유효한 방문 유튜버·관련 영상을 한 사용자 흐름으로 조합한다. WS-02가 여러 영역의 최종 표시 결과를 책임한다.
...
- 빈 결과 또는 실패 처리: 관계나 영상이 없으면 빈 콘텐츠와 기본 정보를 제공한다. 맛집이 없거나 비공개면 찾을 수 없음으로 처리하며 외부 링크 장애가 기본 상세 실패로 확산되지 않게 한다.
...
|---|---|---|---|---|
| PR-DETAIL-001 | 공개 맛집의 기본 정보를 조회한다. | FR-RESTAURANT-008 | Must | 확정 |
| PR-DETAIL-002 | 전체 도로명주소와 필요한 상세 위치를 확인한다. | FR-RESTAURANT-009 | Must | 확정 |
| PR-DETAIL-003 | 대표 음식 카테고리를 확인한다. | FR-RESTAURANT-010 | Must | 확정 |
| PR-DETAIL-004 | 영상 연결이 없어도 맛집 기본 정보를 조회한다. | FR-RESTAURANT-011 | Must | 확정 |
| PR-DETAIL-005 | 유효 관계의 방문 유튜버 표시 정보와 채널 링크를 확인한다. | FR-CREATOR-002 | Must | 확정 |
| PR-DETAIL-006 | 유효 관계의 영상 표시 정보와 원본 링크를 확인한다. | FR-VIDEO-001 | Must | 확정 |

## 11. 비즈니스 규칙

- 맛집의 영상 독립성, 카테고리·지역·공개 조건은 `BR-RESTAURANT-002`, `004`, `005`, `008`을 따른다.
- 유튜버·영상 표시와 이용 불가·외부 장애는 `BR-CREATOR-004`, `007`, `BR-VIDEO-001`, `004`, `007`~`009`를 따른다.
- 관계 연결 범위·조회 유효성은 `BR-VISIT-004`, `005`, 공개 노출은 `BR-PUBLICATION-001`, `003`~`007`을 따른다.
...
조회 성능은 `NFR-PERFORMANCE-001`, 외부 링크 격리는 `NFR-INTEGRITY-004`, `NFR-EXTERNAL-001`, `002`를 따른다. 공통 오류·응답·모바일 크기와 다중 제공자 실패 테스트는 메타데이터의 관련 NFR로 검증한다.

## 14. 의존성

- 선행 정책: 공개 상태 우선순위와 관계 유효성·중복 제거
- 데이터 의존성: WS-04가 등록한 기본 데이터와 관계
- 다른 기능 PRD: PRD-DISCOVERY-002와 동일 Visit 판정 정책 공유
- 다른 Workstream: WS-03 관계 계약, WS-04 등록 결과
- 외부 서비스: 카카오 장소 링크와 YouTube 원본 링크
- 공통 API 계약: 식별자, 표시 정보, 오류와 부분 실패 계약

## 15. Workstream 및 책임자

- 주 Workstream: WS-02 맛집 상세 및 콘텐츠 조회
- 최종 책임자: 박진영
- 기본 리뷰어: 김인안
...
- 네 영역 표시 계약, WS-03 관계 정책과 WS-04 등록 데이터를 통합 검증한다.
...
- 상세 조합의 실제 책임 위치와 내부 제공자 일부 실패 정책이 미확정이다.
...
- 상세 조합의 애플리케이션 책임 위치
- 내부 정보 제공자 실패 시 전체 오류 또는 명시적 부분 결과 정책
