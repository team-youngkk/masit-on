...
- 지도 기반 탐색과 유튜버 상세는 1차 확장 범위에서 검토한다.
...
현재 상세에 표시 가능한 공개·활성·이용 가능 방문만 대상으로 한다. 여러 영상의 태그를 합쳐 하나의 방문 근거로 표시하지 않는다. 별도 관리자 맛집 목록은 범위 밖이다. FR-AIEXTRACT-007·FR-NLSEARCH-004의 사후 보정이며 [구현 계획](../../../08-planning/restaurant-visit-tag-editing.md)과 [관리 API](../../../05-specs/api/admin/restaurant-visit-tags-api.md)를 따른다.

## 관리자 태그 정의 생성 — 이슈 #363

ADMIN은 방문 태그 편집을 벗어나지 않고 새 태그의 유형·코드·표시명·선택적 별칭을 입력할 수 있다. 생성이 성공하면 활성 태그 선택 목록에 즉시 반영하고 현재 편집 중인 방문에서 선택 상태로 둔다. 생성과 방문 연결 저장은 별도 명령이므로, 관리자가 방문 태그 저장을 누르기 전에는 새 `VisitTag`를 만들지 않는다.

익명·MEMBER에게 생성 폼과 내부 태그 코드를 표시하지 않고 관리자 API도 호출하지 않는다. 생성 실패는 입력 필드 또는 충돌 원인을 수정할 수 있게 표시하며 현재 방문의 다른 편집값을 버리지 않는다. 태그 생성 뒤 활성 목록을 다시 조회하되, 사용자가 편집 중인 다른 태그 선택을 덮어쓰지 않는다.

태그 정의 수정·비활성화·재활성화는 전용 [태그 정의 관리 화면](../../../08-planning/admin-tag-definition-lifecycle.md)에서 수행한다. 상세의 기존 비활성 태그에는 상태를 표시하고 유지·제거를 허용하지만 제거 뒤 재선택은 막는다. 병합·물리 삭제는 후속 범위다. 상세 화면 동작은 [태그 정의 API](../../../05-specs/api/admin/tag-definition-api.md)를 따른다.
