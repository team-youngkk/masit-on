...
관리자 회원가입·등급·세분 권한·계정 화면, 데이터 수정·삭제 UI, 승인 워크플로, 별도 검증 상태, 자동 수집·동기화, AI 추출, 영상 원본 저장과 실제 방문 날짜 관리는 목표가 아니다.
...
- 시스템 동작: 권한, 필수값, 사전 정의 값, 외부 링크, 사실 확인과 동일 대상 중복을 검증한다.
