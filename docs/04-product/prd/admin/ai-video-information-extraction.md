...
4. `BR-AIEXTRACT-011`이 정의한 예외 사유에 해당할 때만 보조 입력 화면으로 전환한다. 이때 정식 저장은 0건이다.
...
| PR-AIEXTRACT-015 | 정의된 예외 사유에 해당할 때만 보조 입력 화면으로 전환하고, 그 밖의 경우에는 관리자 입력을 요구하지 않는다. | FR-AIEXTRACT-003, BR-AIEXTRACT-011 |
...
| `AUTO_CONFIRMED` | 등록 단위별 자동 검증·등록·공개 결과, 채택한 Kakao 장소와 카테고리 근거, 버전·시각 | 결과 조회·사후 롤백·카테고리 보정 |
| `AUTO_BLOCKED` | 차단 사유·근거·시각. 장소 판정은 `PLACE_NOT_FOUND`·`PLACE_AMBIGUOUS`, 카테고리는 `CATEGORY_UNRESOLVED`로 구분한다 | 예외 보정·수동 등록 |
| `AUTO_REJECTED` | 거부 사유·근거·시각 | 새 작업 |
| 등록 단위 · 미등록 | 등록 단위 요약과 등록 실행 동작 하나 | 4종 일괄 등록 실행 |
| 등록 단위 · 등록 완료 | 맛집·유튜버·영상·방문 관계 식별자, 채택한 장소와 카테고리 근거 | 결과 조회·사후 롤백·카테고리 보정 |
...
- 관리자가 보완할 수 있는 범위는 맛집명·주소·메뉴·Kakao 장소 URL과 허용 태그 후보의 값·근거 위치로 제한하며, 보완값도 기존 외부 검증을 우회하지 않는다. 보완 텍스트 자체의 방문 주장은 자동 확정 근거가 아니며 실제 영상 `TIMESTAMP`가 있어야 한다.
...
- 후보 수가 수신 상한을 넘는 응답은 `SCHEMA` 실패로 기각하고 등록 단위를 만들지 않는다. 등록 단위 분해는 이 상한을 바꾸지 않으며, 상한 인상은 별도 결정 대상이다. 이 경우 관리자는 보완 텍스트 재시도 또는 기존 수동 등록으로 복구한다.
...
- 현재 운영 계약인 `gemini-3.5-flash-lite`·Prompt `P7`·Schema `S1` 중 하나라도 변경하면 새 후보 버전과 평가 보고서를 만든다.
