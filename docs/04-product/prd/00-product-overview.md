...
- 비공개·삭제·무효 대상의 노출은 `BR-PUBLICATION-001`~`008`을 따른다.
- 공통 규칙의 원문과 우선순위는 [비즈니스 규칙](../../01-requirements/business-rules.md)이 소유한다.

## 13. 공통 품질 목표

- 일반 조회·검색·등록의 목표 시간은 `NFR-PERFORMANCE-001`~`004`에 따라 측정하며 미확정 수치는 확정 전까지 팀 결정 필요로 둔다.
- 공개 조회와 관리자 접근을 분리하고 입력·비밀정보를 보호한다(`NFR-SECURITY-001`~`003`).
- 참조·중복·등록 원자성과 공개 상태 일관성을 보장한다(`NFR-INTEGRITY-001`~`004`).
- 외부 링크 장애를 내부 기본 정보 조회와 격리한다(`NFR-RELIABILITY-001`, `NFR-EXTERNAL-001`~`003`).
- 웹·모바일 브라우저의 핵심 흐름과 자동화 테스트·배포 품질 게이트를 검증한다.

## 14. 기능 PRD 구성

기능 PRD는 WS-01~04의 사용자 가치와 완료 흐름에 맞춰 4개로 구성한다. 탐색 영역은 두 독립 Workstream이므로 별도 PRD를 두고, 상세 콘텐츠는 WS-02로, 관리자 등록 전 과정은 WS-04로 각각 통합한다.

## 15. Workstream 및 책임 구조

| Workstream | PRD | 최종 책임자 | 기본 리뷰어 |
|---|---|---|---|
| WS-01 맛집 탐색 | PRD-DISCOVERY-001 | 양성훈 | 이우람 |
| WS-02 맛집 상세 및 콘텐츠 조회 | PRD-DETAIL-001 | 박진영 | 김인안 |
| WS-03 유튜버 기반 탐색 | PRD-DISCOVERY-002 | 이우람 | 양성훈 |
| WS-04 관리자 데이터 등록 | PRD-ADMIN-001 | 김인안 | 박진영 |
...
- 기능 요구사항 19개의 인수 조건과 적용 NFR 검증이 완료된다.
...
- 관리자 계정은 사전 발급하며 상세 인증 기술은 후속 설계에서 정한다.
...
- WS-04의 등록 통합 범위와 WS-02의 다중 영역 조합이 일정·테스트 복잡도를 높인다.
...
- 관리자 최소 인증 수준과 운영 절차
- 목록에 표시할 유튜버 수와 축약 방식
- 유튜버 선택 목록의 MVP 포함 여부
- 상세 내부 제공자 실패 시 전체 오류와 부분 결과의 경계
