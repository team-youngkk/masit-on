...
- 중요도: Critical
- 현재 상태: 물리 설계 전 결정 필요
- 관련 문서: `scope.md`, BR-RESTAURANT-006·007, API-ADMIN-RESTAURANT-PREVIEW-001
- 영향 데이터: Restaurant
- 영향 API: API-ADMIN-RESTAURANT-PREVIEW-001, API-ADMIN-RESTAURANT-001
...
- 관련 문서: BR-RESTAURANT-008, BR-PUBLICATION-001~008, BR-ADMIN-005·006
...
| 검토 항목 | 결론 | 근거·후속 조건 |
|---|---|---|
| 유튜버 개인 대 채널 | 채널 단위 확정 | scope, BR-CREATOR-001 |
| 지역 관리 단위·계층 | 서울 자치구 참조 데이터, 단일 단계 | glossary, BR-RESTAURANT-005 |
| 다중 음식 카테고리 | 허용하지 않음, 정확히 1개 | BR-RESTAURANT-004 |
| Visit 구조 | Restaurant·Creator·Video 삼항 관계 | BR-VISIT-001, API-ADMIN-VISIT-001 |
| Visit와 게시 채널 일치 | 반드시 일치 | BR-CREATOR-005 |
| 한 방문의 여러 영상 근거 | MVP 미지원 | 세 대상 조합마다 별도 Visit |
| 한 영상의 여러 맛집 | 허용, 맛집별 Visit | BR-VIDEO-004 |
| 재방문 | 다른 Video면 별도 Visit | BR-VISIT-003; 방문일 도입 시 재검토 |
| 영상 없는 Visit | 금지 | BR-VISIT-002 |
