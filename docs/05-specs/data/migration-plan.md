...
### 2.1. 릴리스 전 마이그레이션 통합 정책

[ADR-DATA-004](../../07-adr/data/data-004-flyway.md) 10절은 "적용된 마이그레이션은 수정하지 않고 새 파일로 변경한다"고 정하며 `적용된`의 환경을 한정하지 않는다. 이 문서는 그 범위를 **운영 환경 적용**으로 확정한다.

| 구분 | 규칙 |
|---|---|
| 운영에 적용된 마이그레이션 | 수정·통합·삭제하지 않는다. 변경은 새 버전 파일로만 추가한다 |
| 운영에 아직 적용되지 않은 개발 단계 마이그레이션 | 운영 배포 전에 하나로 통합할 수 있다 |

개발 과정에서 여러 마이그레이션이 누적되면 운영 배포 전에 통합해 baseline을 단순하게 유지한다. 운영 `flyway_schema_history`에는 통합 대상 버전의 기록이 없으므로 통합 결과는 새 마이그레이션으로 적용되며 checksum 충돌이 발생하지 않는다.

통합 시 다음 절차를 지킨다.

1. 통합 대상이 운영에 적용된 적이 없는지 운영 `flyway_schema_history`로 확인한다.
2. 통합 전후 SQL 본문이 동일한지 대조한다.
3. 통합 결과를 빈 데이터베이스에 적용해 스키마와 기준 데이터가 같은지 검증한다.
4. 통합 대상을 이미 적용한 개발·공유 데이터베이스는 볼륨을 재생성한다. 통합을 병합하는 담당자가 팀에 공지한다.

통합 대상을 이미 적용한 데이터베이스는 checksum 불일치와 파일 부재로 Flyway `validate`가 실패한다. 다음으로 재생성한다.
...
초기 스키마는 원래 `V1`~`V5` 다섯 파일이었고 MVP 구현(T-03)과 로컬 검증(T-14)에서 그 형태로 적용됐다. 운영 데이터베이스에 적용된 적이 없으므로 2.1절 정책에 따라 `V1__create_initial_schema.sql` 하나로 통합했다. 적용 결과 스키마와 기준 데이터는 통합 전과 동일하다.
...
`V1`이 운영에 적용된 뒤에는 내용을 고치지 않고 `V2` 이상의 보정 migration을 추가한다. 운영 배포 전이라면 2.1절 정책에 따라 통합할 수 있으며, 이때 통합 대상을 이미 적용한 개발·공유 데이터베이스는 볼륨을 재생성한다.
