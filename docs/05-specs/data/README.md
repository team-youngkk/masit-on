...
| 문서 | 책임 |
|---|---|
| `data-model.md` | 전체 모델, 소유권, 생성·조회 흐름 |
| `entity-definitions.md` | 데이터 개념별 식별·속성·상태·규칙 |
| `relationship-rules.md` | 관계, 카디널리티, Visit 모델 선택 |
| `constraints.md` | 필수값·유일성·참조·원자성 제약 |
| `lifecycle-rules.md` | 공개·외부 상태·삭제를 구분한 생명주기 |
| `data-traceability.md` | PRD·요구사항·규칙·API·Workstream 추적 |
| `data-review.md` | 해결된 결정, 미확정 사항과 ADR 후보 |
| `../diagrams/erd-spec.md` | ERD 포함 범위와 표기 명세 |
...
충돌 시 `scope.md` → 확정 비즈니스 규칙 → 확정 기능 요구사항 → 확정 API 계약 → 기능 PRD → 도메인·Workstream → 검토 항목 순으로 판단한다. 이 디렉터리에서는 전체 모델, 엔티티, 관계, 제약, 생명주기, 추적성, 검토 문서 순으로 읽고 ERD는 마지막에 확인한다.
...
4. `data-traceability.md`와 `data-review.md`를 갱신한다.
...
확정되지 않은 필드나 구현 방식은 임의로 필수화하지 않고 `검토 필요` 또는 `후속 설계에서 결정`으로 표시한다. 문서 충돌과 논리 모델을 막는 질문은 `data-review.md`에 중요도, 영향, 선택지와 결정 시점을 기록한다. 현재 명세의 Open Questions와 Assumptions는 해당 검토 문서가 단일 목록 역할을 한다.
