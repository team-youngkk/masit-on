...
한 작업이 여러 맛집을 등록할 수 있으므로 판정·등록 결과는 Snapshot이 아니라 등록 단위가 소유한다. `V6__add_ai_registration_unit_and_food_category_mapping.sql`로 구현했다.
...
`V6__add_ai_registration_unit_and_food_category_mapping.sql`로 구현했고 seed를 함께 적재했다. 기준정보 소유자는 restaurant 도메인이다.
...
API 3.5절 `review`의 `CONFIRM`·`DISCARD`·`ROLLBACK`·`ADJUST_CATEGORY` 네 `decision`이 만드는 등록 단위 사후 조작을 append-only 이력으로 보존한다. `ai_registration_unit`은 현재 상태만 갖고 있어 반복 보정·행위자·사유를 표현할 수 없으므로 별도 테이블로 분리한다. `V6__add_ai_registration_unit_and_food_category_mapping.sql`로 구현했다.
