...
`parserVersion: P1`은 문장 패턴·조건 해석·충돌·상한·정규화 알고리즘의 버전이다. 동적 사전의 코드·표시명·별칭 데이터 변경만으로 버전을 증가시키지 않는다. V4의 초기 18개 seed와 별칭은 Golden V1 회귀 기준으로 보존하며 동적 전환 전후의 해석 결과가 같아야 한다.
