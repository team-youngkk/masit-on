...
설명, 별도 대표 이미지 URL, 영업 정보와 `기타` 카테고리의 보충 이름은 Restaurant 저장 속성으로 추가하지 않는다. 공개 화면의 대표 이미지는 공개·유효한 관련 Video의 `thumbnailUrl`에서 조회 시 파생한다. 지점명은 필수값이 아니며 카카오 장소와 주소로 지점을 구분한다.
