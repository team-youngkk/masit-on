...
- 보정 실행: Creator별 `QUEUED/RUNNING` 하나, 허용 상태, `MAX_VIDEOS_PER_RUN/MANUAL` 중지 원인, 0 이상 누적 건수, lease 소유자·만료 시각 쌍을 저장소에서 보장하고 활성 `youtube_channel_watch`가 없으면 `STOPPED`로 종결한다.
