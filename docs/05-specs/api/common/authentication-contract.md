...
회원과 관리자 principal, JWT audience, Refresh 쿠키와 Redis namespace는 교차 사용하지 않는다. 반대 audience, 만료·변조 Token과 인증 누락은 `401 AUTHENTICATION_REQUIRED`다. 올바른 경계의 신원으로 인증됐지만 별도 권한이 부족할 때만 `403 FORBIDDEN`을 사용한다.
