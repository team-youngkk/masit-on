...
| `details` | object 또는 null | 아니요 | 기능별 오류 계약이 정의하는 안전한 추가 컨텍스트. 정의되지 않은 오류에서는 생략 |
...
`resource`와 `details`는 서로 다른 목적을 가진다. `resource`는 이미 존재하는 자원의 참조만 담고, 외부 장애·부분 실패처럼 요청의 처리 컨텍스트를 알려야 하는 기능은 해당 기능 문서가 `details`의 구조와 허용값을 정의한다. `details`에도 좌표, 외부 원문 응답, 인증정보와 내부 예외 정보는 담지 않는다.
