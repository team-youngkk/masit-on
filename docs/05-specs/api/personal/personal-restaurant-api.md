...
최근 본 맛집은 별도 공개 쓰기 API 없이 [맛집 상세 API](../detail/restaurant-detail-api.md#api-detail-001-맛집-상세-조회)의 `200 OK` 성공 뒤 서버 내부 부수효과로 기록한다. 공개 상세의 회원 인증은 선택적이며, 인증 또는 개인화 저장소 문제는 공개 조회를 익명으로 격하한다.

- 유효한 회원 인증이 있는 `GET /api/restaurants/{restaurantId}` 성공만 현재 회원과 맛집의 최근 기록을 upsert한다.
- 같은 회원·맛집 조합은 중복 생성하지 않고 `lastViewedAt`만 갱신한다.
- upsert Command 트랜잭션에서는 최신 50개 초과 기록만 정리한다. 30일 경과 기록은 회원 조회와 무관한 주기 cleanup Command가 삭제한다.
- Token 누락·만료·변조·폐기, 회원 인증 저장소 장애와 기록 저장 실패는 공개 맛집 상세를 익명 `200`으로 유지하고 최근 기록만 생략한다.
- 비로그인·익명으로 격하된 조회, `404`, `500`, 비공개·삭제 맛집 조회는 기록하지 않는다.
...
- 삭제된 맛집의 찜과 탈퇴 회원의 찜은 맛집·회원 삭제 Command 또는 FK 정책에서 정리한다. 목록 조회는 삭제하지 않고 공개·유효 관계만 필터링한다.
...
- 목록 조회는 마지막 조회 후 30일 이내의 최신 50개 기록만 필터링하며 삭제를 수행하지 않는다. 보존 초과 정리는 회원 조회와 독립된 주기 cleanup Command에서 처리한다.
...
| 삭제 맛집 찜 관계 | 삭제 Command/FK 정책에서 정리되며, 정리 전에도 목록에는 노출되지 않음 |
