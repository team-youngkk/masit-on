...
- `placeDecision.matchedBy`와 `categoryDecision.resolvedBy`는 자동 판정 근거다. 새 실행의 `matchedBy`는 exact-name 우선 경로의 `NAME_AND_DISTRICT` 또는 exact 후보가 없을 때 제한적으로 허용되는 양방향 이름 containment/LIKE fallback 경로의 `NAME_CONTAINMENT_AND_DISTRICT_AND_CATEGORY`다. PR #278에서 이미 저장된 `NAME_CONTAINS_AND_DISTRICT_AND_CATEGORY`는 legacy 감사 값으로 조회 응답에서 계속 허용하며 새 실행에서는 생성하지 않는다. fallback은 원본 상호명과 명시적 지점 표기(`본점`, `본관`, `별관`, `지점`, `1호점` 등)를 제거한 기본 상호명을 검색하고, Kakao 분류와 AI 메뉴가 같은 단일 음식 카테고리로 매핑되며, 시·구가 일치하고, 장소 동일성 판정 필수 필드가 있고, qualifying candidate가 정확히 1건일 때만 응답에 나타난다. 좌표는 지도 표시용 선택값이다. `resolvedBy`는 `KAKAO_PLACE_CATEGORY` 또는 `MENU_EXPRESSION`이며, 관리자 사후 보정 결과는 `MANUAL_OVERRIDE`로 표시한다.
...
`blockReason`·`validationFailureReason`·`recoveryPaths`·`requiredSupplements`는 [공통 오류 계약](../common/error-contract.md)의 `details` 필드 안에 중첩된다. `code`·`traceId`와 같은 최상위 필드가 아니다.
