...
입력이 유효하면 [공통 접수 응답](#4-계정-상태-비노출-공통-응답)을 반환한다. 신규 가입 가능한 경우 미인증 회원과 24시간 유효한 최신 일회용 인증 Token을 만들고 메일을 발송한다. 정규화 이메일당 직전 요청 60초 뒤부터 하루 최대 5회, 회원가입·재설정 합산 요청 출처당 시간당 최대 20회를 원자적으로 적용한다. 중복·활성·탈퇴 정리 중 계정이나 제한 초과 요청은 외부에 구분하지 않으며 새 계정·Token·메일을 만들지 않는다.

### API-MEMBER-AUTH-002 가입 이메일 인증

- Method: `POST`
- Path: `/api/auth/email-verifications`
- 인증: 요청 본문의 가입 이메일 인증 Token
...
발급 후 24시간 안의 아직 사용하지 않은 최신 Token이면 일회성으로 소비하고 회원을 활성 상태로 전환한 뒤 `204 No Content`를 반환한다. 누락은 `400 MISSING_REQUIRED_FIELD`, 변조·만료·재사용·이전 발급·탈퇴 회원 Token은 모두 `400 INVALID_EMAIL_VERIFICATION_TOKEN`으로 처리한다. 같은 Token의 동시 요청은 하나만 `204`이며 나머지는 동일한 `400`이다.
...
입력이 유효하면 [공통 접수 응답](#4-계정-상태-비노출-공통-응답)을 반환한다. 미인증 회원이면서 직전 발송 후 60초가 지났고 이메일당 하루 5회 이하인 경우에만 새 24시간 Token을 발급하고, 메일 발송이 접수된 뒤 이전 Token을 무효화한다. 계정 상태, 실제 발송과 제한 초과 여부는 응답에서 구분하지 않는다.
...
- 이메일 인증 24시간, 비밀번호 재설정 30분, Access Token 30분과 Refresh Token 14일 경계를 검증한다.
...
| API-MEMBER-AUTH-002 | FR-MEMBER-002 | BR-MEMBER-003 | NFR-SECURITY-004, NFR-TEST-004 |
