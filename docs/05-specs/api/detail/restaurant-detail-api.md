...
- 공개·유효한 영상이 근거인 관계만 `visitedBy`와 `videos`에 반영한다. 비공개·삭제·외부 이용 불가 영상이 근거인 관계는 전체를 제외하며, 그런 관계만 있으면 두 목록은 모두 `[]`다.
...
| 공개·유효한 영상을 근거로 한 관계 없음 | `200`, `contentStatus: AVAILABLE`, `visitedBy: []`, `videos: []` |
