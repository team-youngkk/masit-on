...
|---|---|---|---|
| API-CREATOR-DISCOVERY-001 | GET | `/creators` | 유튜버 필터 최소 선택 목록 |
| API-DISCOVERY-001 | GET | `/restaurants?creatorId=...` | 특정 유튜버의 방문 맛집을 최종 목록으로 조회 |
...
FR-CREATOR-001의 주 외부 API는 API-DISCOVERY-001이다. WS-03은 `creatorId`에 대해 유튜버·영상·관계·맛집이 모두 공개·유효하고, 영상 게시 채널이 유튜버와 일치하며, 실제 방문 근거가 있는 고유 맛집만 판정한다. WS-01은 그 결과를 다른 조건과 AND 조합하고 정렬·페이지 처리한다.
...
- PRD-DISCOVERY-002, WS-03, 이우람
- FR-CREATOR-001, FR-CREATOR-003
- BR-CREATOR-001·004·005·007, BR-VIDEO-005·009, BR-VISIT-001~007, BR-SEARCH-003~007
