...
--   순번 1~50    (상위 50개)  : 맛집당 (226 - r)건 (225~176건) -> 10,025건 (상위 20개 찜 수 차별화)
--   순번 51~200  (다음 150개) : 맛집당 40건                  ->  6,000건
--   순번 201~225 (보정 25개)  : 맛집당 4건                   ->    100건
--   순번 226~1000(나머지 775개): 맛집당 5건                   ->  3,875건
--   합계 = 10,025 + 6,000 + 100 + 3,875 = 20,000건 (요구 건수와 정확히 일치)
-- 맛집당 최댓값(225)은 회원 수 상한(1,000, 스키마 사실 10번)보다 한참 낮아 상한을
-- 위반하지 않는다.
--
-- 맛집 r의 j번째(1-base) 찜을 받는 회원은 member_idx = ((r + j - 1) % 1000) + 1로 고른다.
-- 맛집마다 j=1..tc(r)를 순회하는 동안 member_idx는 서로 달라(mod 1000, tc(r) <= 225 < 1000)
