...
이 모듈은 앱 EC2와 분리된 전용 Redis를 사설 subnet에 배치한다. 앱은 단일 EC2에서 실행되며 Redis는 세션·Refresh Token·rate-limit 상태를 공유하는 별도 endpoint다. 실제 Redis 데이터 이전·cutover·복구는 운영 runbook과 리허설을 통과한 뒤 수행한다.

`../terraform`(운영 단일 앱 EC2)과 state를 분리한다. 그 모듈은 `redis_security_group_id`를 **입력으로만** 받고 Redis를 만들지 않는다. 이 모듈의 `redis_security_group_id` 출력을 그 입력에 넣는다.
