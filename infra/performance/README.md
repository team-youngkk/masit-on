...
| 검증 세션·gate | M2에서 사용했던 제한 공개 경계 | 현재 제거됨 | 성능 측정은 제품 API의 인증 계약과 무인증 공개 GET만 대상으로 하며 별도 검증 로그인·쿠키·gate 흐름은 측정하지 않는다 |
...
회원 인증 origin은 운영과 같은 값을 쓴다. `prod` 프로파일이 HTTPS를 강제해(`Production authentication origins must use HTTPS`) 사설 주소를 넣으면 기동하지 않는다. M2 제한 공개의 검증 세션·쿠키·gate는 정식 공개 전환으로 제거됐으므로 성능 환경에서도 별도 검증 세션 HTTP 매핑을 등록하지 않는다.
