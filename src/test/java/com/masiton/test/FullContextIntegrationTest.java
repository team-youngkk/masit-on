...
public abstract class FullContextIntegrationTest {
    ...
    public static void cleanupTransactionalState(org.springframework.jdbc.core.JdbcTemplate jdbcTemplate) {
        jdbcTemplate.execute("TRUNCATE TABLE ai_candidate_tag_review, ai_extraction_manual_review, "
                + "visit_tag_merge_provenance, tag_definition_merge, visit_tag_revision, tag_definition_audit");
    ...
    private static void restoreSeedTagAliases(org.springframework.jdbc.core.JdbcTemplate jdbcTemplate) {
        jdbcTemplate.execute("""
                WITH seed_aliases(tag_code, aliases) AS (
                    VALUES
                        ('MENU_NAENGMYEON', '["물냉면", "비빔냉면"]'::jsonb),
                        ('MENU_GUKBAP', '[]'::jsonb), ('MENU_RAMEN', '[]'::jsonb),
                        ('MENU_SUSHI', '["초밥"]'::jsonb), ('MENU_PIZZA', '[]'::jsonb),
                ...
                """);
        jdbcTemplate.execute("""
                DELETE FROM tag_definition_term
                 WHERE term_kind = 'ALIAS'
                   AND tag_definition_id IN (SELECT id FROM tag_definition WHERE source = 'SEED')
                """);
        jdbcTemplate.execute("""
                ...
                SELECT md5(definition.id::text || ':ALIAS:' || alias.ordinality::text)::uuid,
                       definition.id, 'ALIAS', normalize_tag_definition_term(alias.value), definition.created_at
